...
//上锁
static bool VXMutexBase_lock(XSemaphoreWin32* mutex);
static bool VXMutexBase_lock_wait(XSemaphoreWin32* mutex, size_t timerout);
//解锁
static bool VXMutexBase_unlock(XSemaphoreWin32* mutex);
...
void XSemaphoreWin32_init(XSemaphoreWin32* mutex, const char* name)
{
    if (mutex == NULL)
        return;
    memset(((XMutexBase*)mutex) + 1, 0, sizeof(XSemaphoreWin32) - sizeof(XMutexBase));
    XMutexBase_init(mutex, NULL);
    XClassGetVtable(mutex) = XSemaphoreWin32_class_init();
    // 创建信号量（初始时无人拥有）
    mutex->m_mutex = CreateSemaphore(
        NULL,                // 安全属性，通常为 NULL
        1,                   // 初始计数（1 表示可用）
...
bool VXMutexBase_lock(XSemaphoreWin32* mutex)
{
    return WaitForSingleObject(mutex->m_mutex, INFINITE) == WAIT_OBJECT_0;
}

bool VXMutexBase_lock_wait(XSemaphoreWin32* mutex, size_t timerout)
{
    return WaitForSingleObject(mutex->m_mutex, timerout) == WAIT_OBJECT_0;
}

bool VXMutexBase_unlock(XSemaphoreWin32* mutex)
{
    return  ReleaseSemaphore(mutex->m_mutex,1, NULL);
}
#endif