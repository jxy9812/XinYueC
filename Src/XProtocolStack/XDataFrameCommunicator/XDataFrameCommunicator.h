#ifndef XDATAFRAMECOMMUNICATOR_H
#define XDATAFRAMECOMMUNICATOR_H
#ifdef __cplusplus
extern "C" {
#endif
#include<stdint.h>
...
#define XDATAFRAMECOMMUNICATOR_VTABLE_SIZE		(XCLASS_VTABLE_GET_SIZE(XDataFrameCommunicator))       //XDataFrameCommunicator虚函数表大小
XCLASS_DEFINE_BEGING(XDataFrameCommunicator)
XCLASS_DEFINE_ENUM(XDataFrameCommunicator, SendFrame) = XCLASS_VTABLE_GET_SIZE(XCommunicatorBase),
XCLASS_DEFINE_ENUM(XDataFrameCommunicator, RecvFrame),
XCLASS_DEFINE_END(XDataFrameCommunicator)
typedef enum
{
    XDFC_READY,                   /*!< 启动完成事件 */
...
//数据帧通信类
typedef struct XDataFrameCommunicator
{
