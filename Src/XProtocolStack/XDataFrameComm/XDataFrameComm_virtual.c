...
void XDataFrameComm_recvValid(XDataFrameComm* comm)
{
	...
	{//帧验证失败,错误帧
		XDataFrameComm_postEvent(comm, XEvent_create(XDFC_RX_FRAME_ERROR));
		return;//校验没通过
	}
...
void VXDataFrameComm_SendFrameFSM(XDataFrameComm* comm)
{
	...
	switch (comm->m_eSndState)
	{
		...
		case XDFC_STATE_TX_XMIT:  // 发送中状态（逐个字节发送）
		{
			...
			XDataFrameComm_postEvent(comm, XEvent_create(XDFC_FRAME_SENT));
			break;
		}
	}
}
...
static void TimerSendExpired(XDataFrameComm* comm)
{
	...
	//XPrintf("发送完一帧数据\n");
	{
		...
		//XObject_postFunc(comm, SendExpired, comm,NULL, XEVENT_SEND_DIRECT, XEVENT_PRIORITY_NORMAL);
	}
}
...
static void RecvExpired(XDataFrameComm* comm)
{
	...
	case XDFC_STATE_RX_INIT:  // 初始状态超时（总线空闲，进入IDLE）
		XDataFrameComm_postEvent(comm, XEvent_create(XDFC_READY));
		break;
...
{  
	...
	//XObject_postFunc(comm, RecvExpired, comm, NULL,XEVENT_SEND_DIRECT, XEVENT_PRIORITY_NORMAL);
}
