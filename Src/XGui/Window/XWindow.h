...
/**
 * @brief      窗口事件总入口（对标 QWindow::event；虚表内为 EXObject_Event
 *             槽位，XWindow 已重载为默认分发器）。
 * @details    按 XEvent_type 路由到具体事件槽（expose/resize/paint/move/
 *             focusIn/focusOut/show/hide/close/key/mouse/wheel/touch/tablet）；
 *             未识别事件回退调用 XObject 默认 Event 实现。
 * @param      self 目标窗口；可为 NULL。
