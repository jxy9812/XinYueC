...
/* 连续「无快速路径」命令共享一张全帧透明暂存画布：批内各命令由软件
   光栅直接画在暂存画布上（画布透明底、只含本批输出），失效点一次
   「脏区 SourceOver 混合提交」回 FBO——不再做批首 FBO 快照（省 3.2ms/
   次全帧读回），脏区通道（drawImageRegion 增量上传）不可用时回退整幅
   SourceOver 提交（正确性不变，带宽较差）。语义依据 Porter-Duff Over
   结合律：批内命令两两 SourceOver → （S2 over S1）over D ≡ 逐命令直绘。
   目标依赖命令（RasterOp/非 SourceOver 合成，快照不可省）冲批后走原
   逐命令 legacy 路径。XGUI_GPU_SYNC=1 调试口径不变。 */
static XPainter* g_gpuBatchPainter = NULL; /**< 批内待提交命令属主。 */
static XGpuRenderBackend* g_gpuBatchBackend = NULL; /**< 批首会话（防串会话提交）。 */
static XImage g_gpuBatchCanvas;              /**< 持久全帧透明暂存画布（ARGB32）。 */
...
static void xgpu_batch_canvas_clear_rows(void)
{
    ...
    if (fullRowMode < 0)
    {
        ...
        const char* fr = XSystem_environment("XGPU_CLEAR_ROWS_FULL");
        fullRowMode = fr && *fr && !(fr[0] == '0' && fr[1] == 0) ? 1 : 0;
    }
 ...
 *  @note  批内裁剪/合成已由软件光栅在透明画布上完成；提交用 SourceOver
 *         混合（画布脏区外透明像素对既有内容幂等），不再要求整幅覆盖。
 *         可重复调用，无待定批时直返。 */
static void painterGpuBatchFlush(void)
{
    ...
    if (g_gpuBatchHasDirty && dirty.width > 0 && dirty.height > 0)
    {
...
/* 嵌套帧跟踪：同会话同目标的 painter 复用已开启的 GPU 帧（每控件
   paintEvent 各自 begin/end 是软件路径的零成本设计，GPU 直通下放大为
   每帧 ~45 对 FBO 绑定+状态复位——图表页实测成为框架级主开销之一）。 */
...
static void painterGpuEndFrame(XPainter* self)
{
    ...
    if (self->m_gpuBackend)
    {
        ...
        {
            /* 嵌套内层结束：帧保持开启（外层 painter 收口时统一
               readback/endFrame）。提前冲批不破坏顺序——批内容本就
               属于本帧更早的命令。会话占用标志保持（外层仍在用）。 */
            --g_gpuFrameDepth;
...
/**
 * @brief      批量版软件局部提交（脏区变体）。
 * @param      cmdRect 命令写入外接矩形（设备/画布坐标，可为 NULL——
 *             此时以裁剪包围盒近似；软件光栅写入恒在裁剪内）。
 * @return     true 命令已上暂存画布（或已按原路径提交）；false 会话
 *             无效且原路径亦失败。
 */
static bool painterGpuSubmitSoftwareCommandRect(XPainter* self,
    void (*drawCommand)(XPainter* self, void* userData), void* userData,
    const XRect* cmdRect)
{
    XGpuRenderBackend* backend;
    ...
    if (!painterGpuSyncRequested())
    {
        /* 目标依赖命令（RasterOp/非 SourceOver 合成）：透明暂存语义
           不成立（等价性依赖 Over 结合律），冲批后走原逐命令路径。 */
        if (self->m_state.m_compositionMode !=
        ...
        if (g_gpuBatchActive)
        {
            /* 脏区累计：命令几何 ∩ 裁剪包围盒（无命令几何时取裁剪包围
               盒——软件光栅写入恒在裁剪内，裁剪外的画布像素保持透明）。 */
            XRect bound;
            bool hasBound = false;
#if XPAINTER_CLIP_ON
            if (self->m_state.m_hasClip && self->m_state.m_hasClipRect)
            {
                bound = self->m_state.m_clipRect;
                hasBound = true;
            }
#if XPAINTER_CLIP_REGION_ON
            if (!hasBound && self->m_state.m_hasClip &&
                self->m_state.m_clipRegion.count > 0)
            {
                XRegion_boundingRect(&self->m_state.m_clipRegion, &bound);
                hasBound = true;
            }
            ...
            if (cmdRect)
            {
                ...
                {
                    bound = XRect_intersected(&bound, cmdRect);
                }
            }
            else if (!hasBound)
            {
                /* 既无命令几何也无裁剪：软件光栅写入上界=整幅画布。
                   不得留空——脏区不累计会使冲批整幅提交被跳过，本批
            ...
            if (hasBound && bound.width > 0 && bound.height > 0)
            {
                ...
                if (bound.y + bound.height > ch) bound.height = ch - bound.y;
                if (bound.width > 0 && bound.height > 0)
                {
                    if (!g_gpuBatchHasDirty)
                    {
                        g_gpuBatchDirty = bound;
                        g_gpuBatchHasDirty = true;
                    }
                    else
                    {
                        int x0 = g_gpuBatchDirty.x < bound.x
                                     ? g_gpuBatchDirty.x : bound.x;
                        int y0 = g_gpuBatchDirty.y < bound.y
                                     ? g_gpuBatchDirty.y : bound.y;
                        int x1 = g_gpuBatchDirty.x + g_gpuBatchDirty.width;
                        int y1 = g_gpuBatchDirty.y + g_gpuBatchDirty.height;
                        if (bound.x + bound.width > x1)
                            x1 = bound.x + bound.width;
                        if (bound.y + bound.height > y1)
                            y1 = bound.y + bound.height;
                        g_gpuBatchDirty.x = x0;
                        g_gpuBatchDirty.y = y0;
                        g_gpuBatchDirty.width = x1 - x0;
                        g_gpuBatchDirty.height = y1 - y0;
                    }
                }
            }
            savedImage = self->m_image;
 ...
 *             令共享持久全帧透明暂存画布——批内命令软件画上画布（
 *             m_image 临时切换、m_gpuActive 临时关闭，与纯软件渲染
 *             逐像素同源），提交延迟到失效点（painterGpuBatchFlush：
 *             下一条原语直呼/帧末/整帧降级）一次脏区 SourceOver 混合
 *             提交。XGUI_GPU_SYNC=1 时退回原逐命令路径（每命令立即
 *             可见的调试契约）；批量 setup 失败同样退回，行为不劣化。
 * @param      self 绘制器（须 GPU 会话激活）。
 * @param      drawCommand 命令执行回调（重入对应命令的软件实现；
 *             执行期间 m_gpuActive 为 false，不会再次进入本函数）。
 * @param      userData 命令参数（借用）。
...
static bool painterGpuSubmitSoftwareCommand(XPainter* self,
    void (*drawCommand)(XPainter* self, void* userData), void* userData)
{
    return painterGpuSubmitSoftwareCommandRect(self, drawCommand, userData,
                                               NULL);
}
...
/* 移位半像素约定（像素 px 覆盖 [px-0.5, px+0.5)）的像素范围：字形灰度
   ...
   不会错画。 */
static bool painterSpanPixelRangeHalfOpen(float left, float right,
                                          int* outFirst, int* outLast)
{
    if (!painterSpanPixelRange(left, right, outFirst, outLast))
        return false;
    if (*outLast < INT_MAX) *outLast += 1;
    return *outFirst <= *outLast;
}
...
static bool painterRegionsEqual(const XRegion* lhs, const XRegion* rhs)
{
    ...
    return XMemcmp(lhs->rects, rhs->rects,
                                         ...
                                         int ix2, int iy2)
{
    ...
    if (self->m_gpuActive)
    {
           ...
           端点已是设备坐标，重入 painterRaster_drawLineDevice 不再映射。 */
        {
            ...
            int lx1 = (ix1 > ix2 ? ix1 : ix2) + (int)width / 2 + 2;
            int ly1 = (iy1 > iy2 ? iy1 : iy2) + (int)width / 2 + 2;
            lineRect.x = lx0;
            lineRect.y = ly0;
            lineRect.width = lx1 - lx0 + 1;
            lineRect.height = ly1 - ly0 + 1;
            return painterGpuSubmitSoftwareCommandRect(
                self, painterGpuDrawLineDeviceCommand, &args, &lineRect);
        }
    }
#endif /* XPLATFORMINTEGRATION_ON && XGPU_ON */
    start = -(width / 2);
                                    ...
                                    int x, int y)
{
    ...
    if (self->m_gpuActive)
    {
        ...
        if (painterMatrixIsIdentity(&transform))
        {
            ...
            {
#if XPAINTER_CLIP_REGION_ON
                /* 单矩形 region 可用 scissor（与 fillRect 快路径同口径）：
                   表面裁剪恒以单矩形 region 播种（begin_image），此前
                   一律拒收使控件缓存贴图系统性落入软件局部提交
                   （2026-09-24 复核确认 #32）。 */
                if (!XRegion_isEmpty(&state->m_clipRegion) &&
                    state->m_clipRegion.count > 1)
                    clipOk = false;
...
void XPainter_setBackground(XPainter* self, uint32_t color)
{
    ...
    XMemset(&self->m_state.m_backgroundBrush.m_gradient, 0,
                                  ...
                                  XPainterFillRule fillRule)
{
    ...
    /* 半透明实色 span 快路径判定（P2 复扫 R-72 附带优化）：SourceOver
       + 半透明 alpha + ARGB32 目标 + 无图案/无路径裁剪/单矩形裁剪/
    ...
    int fastBpl = 0;
    {
        if (!gradient && !bulkSolid &&
            ...
            ((solidColor >> 24) & 255u) != 255u &&
            XImage_format(self->m_image) == XImageFormat_ARGB32 &&
            !painterPatternActive(self) &&
            !self->m_state.m_hasClipPath &&
            self->m_state.m_clipRegion.count <= 1 &&
            !g_surfaceClipActive)
        {
            fastBlend = 1;
            fastSa = (solidColor >> 24) & 255u;
            fastSr = (solidColor >> 16) & 255u;
            fastSg = (solidColor >> 8) & 255u;
            fastSb = solidColor & 255u;
            fastIsr = 255u - fastSa;
            fastSpR = painterMul255(fastSr, fastSa);
            fastSpG = painterMul255(fastSg, fastSa);
            fastSpB = painterMul255(fastSb, fastSa);
            fastBits = XImage_bits(self->m_image);
            fastBpl = XImage_bytesPerLine(self->m_image);
            /* 行界收口到裁剪矩形（等价 putPixel 的逐像素 y 判定）。 */
            if (self->m_state.m_hasClip)
            {
                if (py0 < self->m_state.m_clipRect.y)
                    py0 = self->m_state.m_clipRect.y;
                if (py1 >= self->m_state.m_clipRect.y +
                               self->m_state.m_clipRect.height)
                    py1 = self->m_state.m_clipRect.y +
                          self->m_state.m_clipRect.height - 1;
            }
        }
    }
    for (py = py0; py <= py1; ++py)
    {
            ...
            for (j = 0; j + 1 < spanCount; j += 2)
        {
            ...
            else if (fastBlend && xl <= xr)
            {
                /* 半透明实色 span 快路径：putPixel 的四层裁剪判定与
                   函数调用开销提升到行级/调用级（裁剪矩形 x/y 收口），
                   compose SourceOver 通用分支内联展开——混合数学与
                   painterComposeColor 逐位一致。 */
                uint8_t* rowBase = fastBits +
                                   (size_t)py * (size_t)fastBpl;
                int fx0 = xl;
                int fx1 = xr;
                int px2;
                if (self->m_state.m_hasClip)
                {
                    if (fx0 < self->m_state.m_clipRect.x)
                        fx0 = self->m_state.m_clipRect.x;
                    if (fx1 >= self->m_state.m_clipRect.x +
                                   self->m_state.m_clipRect.width)
                        fx1 = self->m_state.m_clipRect.x +
                              self->m_state.m_clipRect.width - 1;
                }
                for (px2 = fx0; px2 <= fx1; ++px2)
                {
                    uint8_t* d = rowBase + (size_t)px2 * 4u;
                    unsigned da = d[3];
                    unsigned dpR = painterMul255(d[2], da);
                    unsigned dpG = painterMul255(d[1], da);
                    unsigned dpB = painterMul255(d[0], da);
                    unsigned outA = fastSa + da;
                    unsigned outPR;
                    unsigned outPG;
                    unsigned outPB;
                    unsigned outR;
                    ...
                    outPR = fastSpR + painterMul255(dpR, fastIsr);
                    outPG = fastSpG + painterMul255(dpG, fastIsr);
                    outPB = fastSpB + painterMul255(dpB, fastIsr);
                    outR = painterUnpremultiply(outPR, outA);
                    outG = painterUnpremultiply(outPG, outA);
                    outB = painterUnpremultiply(outPB, outA);
                    d[0] = (uint8_t)outB;
    ...
    XPainterFillRule fillRule)
{
    ...
    for (py = y0; py <= y1; ++py)
    {
        ...
        int pxFirst = width;
        int pxLast = -1;
        XMemset(counts, 0, (size_t)width * sizeof(*counts));
        for (sub = 0; sub < subdiv; ++sub)
        {
            ...
            }
            {
                ...
                for (j = 0; j + 1 < spanCount; j += 2)
                {
                    ...
                    int spanLast;
                    int px;
                    if (!painterSpanPixelRangeHalfOpen(spans[j],
                                                       spans[j + 1],
                                                       &spanFirst, &spanLast))
                        continue;
...
bool XPainter_begin_image(XPainter* self, XImage* image)
{
    ...
    if (g_surfaceClipActive) {
        static int p4On2 = -1;
        if (p4On2 < 0) {
            const char* v = XSystem_environment("XGPU_P4LOG");
            p4On2 = v && *v && !(v[0] == '0' && v[1] == 0) ? 1 : 0;
        }
        if (p4On2)
            fprintf(stderr, "P4[sclip] active=%d imgMatch=%d rect=%d,%d %dx%d\n",
                    g_surfaceClipActive ? 1 : 0,
                    (image == g_surfaceClipImage) ? 1 : 0,
                    g_surfaceClipRect.x, g_surfaceClipRect.y,
                    g_surfaceClipRect.width, g_surfaceClipRect.height);
    }
        ...
        XImage_width(image) > 0 && XImage_height(image) > 0)
    {
        ...
        {
            /* 嵌套 painter（同会话同目标图像）：帧已开启，跳过重复
               beginFrameImage——软件路径每控件 painter 零成本的设计在
               GPU 直通下放大为每帧 ~45 对 FBO 绑定+状态复位+离屏重复
               上传（框架级主开销，2026-09-24 嵌套帧改造）。 */
        ...
        else if (gpu && XGpuRenderBackend_beginFrameImage(gpu, image))
        {
            ...
            g_gpuFrameActiveImage = image;
            g_gpuFrameDepth = 1;
            /* 帧真正开启（深度 0→1，每帧一次）：批量暂存画布整幅清零。
               画布跨 painter 复用，冲刷后的清行依赖脏区覆盖全部写入——
               一旦有命令写入逃过脏区记录（字形边缘/内部直写等），残留
               会污染后续同尺寸批的提交（QSS 盒模型中心像素污染实锤，
               2026-09-24）。整幅清零=每帧一次 memset，换取确定性正确。 */
            if (g_gpuBatchCanvasInited &&
...
/* 缺字形 ASCII 代理（统一字体回退链的最后一环）：内置 XFontOutlineCommon
   仅覆盖 ASCII + GB2312 一级汉字，全角标点/破折号/箭头等常用符号缺字形，
   此前表现是 M 宽空白占位不画（advance 有、墨迹无）。本表把常用缺字形
   码点映射到字库必有的 ASCII 代理字形（对标 Qt 无候选字体做字形合并时
   的可见回退）；调用方必须先用原码点查字库、失败才落到这里——外挂系统
   字库收录这些字形时不受影响。代理仍未收录的码点维持既有空白占位口径。 */
static uint32_t painterGlyphProxyCodepoint(uint32_t cp)
{
    /* 全角 ASCII 形态（＃Ａｚ　！＂…）整体平移映射，覆盖 FF01-FF5E。 */
    if (cp >= 0xFF01u && cp <= 0xFF5Eu)
        return cp - 0xFEE0u;
    /* 圆序号 ①-⑨ 代理数字。 */
    if (cp >= 0x2460u && cp <= 0x2468u)
        return cp - 0x2460u + (uint32_t)'1';
    switch (cp)
    {
        /* CJK 标点/括号。 */
        case 0x3001u: return (uint32_t)',';   /* 、 */
        case 0x3002u: return (uint32_t)'.';   /* 。 */
        case 0x300Au: return (uint32_t)'<';   /* 《 */
        case 0x300Bu: return (uint32_t)'>';   /* 》 */
        case 0x300Cu: return (uint32_t)'[';   /* 「 */
        case 0x300Du: return (uint32_t)']';   /* 」 */
        case 0x3010u: return (uint32_t)'[';   /* 【 */
        case 0x3011u: return (uint32_t)']';   /* 】 */
        /* 破折号/连接号（U+2010-U+2015 统一代理连字符）。 */
        case 0x2010u:
        case 0x2011u:
        case 0x2012u:
        case 0x2013u:
        case 0x2014u:
        case 0x2015u: return (uint32_t)'-';
        /* 曲引号。 */
        case 0x2018u:
        case 0x2019u: return (uint32_t)'\'';
        case 0x201Cu:
        case 0x201Du: return (uint32_t)'"';
        /* 省略号/项目符号/间隔号。 */
        case 0x2022u:
        case 0x2026u:
        case 0x00B7u: return (uint32_t)'.';
        /* 箭头（ASCII 尖括号近似）。 */
        case 0x2190u: return (uint32_t)'<';
        case 0x2191u: return (uint32_t)'^';
        case 0x2192u: return (uint32_t)'>';
        case 0x2193u: return (uint32_t)'v';
        /* 常用符号单字形代理。 */
        case 0x00A7u: return (uint32_t)'S';   /* § */
        case 0x00D7u: return (uint32_t)'x';   /* × */
        case 0x2212u: return (uint32_t)'-';   /* − */
        case 0x2248u: return (uint32_t)'~';   /* ≈ */
        default: return cp;
    }
}
                                    ...
                                    float shear)
{
                                         ...
                                         &sink))
    {
        /* 缺字形代理回退：原码点字库未收录时改画 ASCII 代理字形（见
           ...
           返回。路径按原码点入缓存（键不变，数据自洽）。 */
        uint32_t proxy = painterGlyphProxyCodepoint(codepoint);
        if (proxy == codepoint)
        {
                                   ...
                                   float scale, uint32_t cp)
{
    ...
    if (painterGlyphAdvanceCacheGet(face, cp, scaleKey, &cached))
        return cached;
#endif /* XFONT_OUTLINE_ON && XPAINTER_PATH_ON */
    if (XFontFace_loadBitmapGlyph_base(face, font, cp, &dsc, NULL, 0))
    {
        cached = painter8x16GlyphAdvance(&dsc, table, scale);
    }
    else
    {
        /* 缺字形代理回退（与绘制路径同口径）：点阵字库未收录的码点
           先试 ASCII 代理字形步进，仍失败按表宽空白占位。 */
        uint32_t proxy = painterGlyphProxyCodepoint(cp);
        if (proxy != cp &&
            XFontFace_loadBitmapGlyph_base(face, font, proxy, &dsc, NULL, 0))
            cached = painter8x16GlyphAdvance(&dsc, table, scale);
        else
            cached = painter8x16Metric(table ? table->m_width : 0, scale);
    }
                             ...
                             size_t dataSize)
{
    XFontFaceInfo faceInfo;
    const XFontFace* face;
    if (!table || !dsc || !data || !(face = XFont_face(font)))
        return false;
    if (!XFontFace_loadBitmapGlyph_base(face, font, cp, dsc, data, dataSize))
    {
        /* 缺字形代理回退（与 outline 路径同口径）：点阵字库未收录的
           码点改用 ASCII 代理字形。 */
        uint32_t proxy = painterGlyphProxyCodepoint(cp);
        if (proxy == cp ||
            !XFontFace_loadBitmapGlyph_base(face, font, proxy, dsc, data,
                                            dataSize))
            return false;
    }
    XMemset(&faceInfo, 0, sizeof(faceInfo));
...
static bool painterPathVerticesPush(PainterPathVertices* v, float x, float y)
{
    ...
    if (v->m_count >= v->m_capacity)
    {
        ...
        else
        {
            ...
            if (v->m_count > 0)
            {
                XMemcpy(nx, v->xs, (size_t)v->m_count * sizeof(float));
                XMemcpy(ny, v->ys, (size_t)v->m_count * sizeof(float));
            }
            v->xs = nx;
            v->ys = ny;
    ...
    const PainterPathVertices* vertices, bool closed)
{
    ...
    XMemcpy(xs, vertices->xs, bytes);
    XMemcpy(ys, vertices->ys, bytes);
...
void XPainter_setBrush(XPainter* self, uint32_t color)
{
    ...
    XMemset(&self->m_state.m_brush.m_gradient, 0,
...
void XPainterGradient_addStop(XPainterGradient* g, float position, uint32_t color)
{
    ...
    if (g->m_stopCount >= XPAINTER_GRADIENT_MAX_STOPS)
        return;
    XMemmove(&g->m_stops[index + 1], &g->m_stops[index],
...
{
    if (!self || self->m_deviceKind == XPainterDevice_None) return;
    XMemset(&self->m_state.m_transform, 0, sizeof(XImageTransform));
