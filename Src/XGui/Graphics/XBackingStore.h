 ...
 *             - resize() 重建缓冲；beginPaint()/endPaint() 界定绘制区间；
 *               scroll() 在缓冲内位移；setStaticContents()/staticContents()/
 *               hasStaticContents() 标记受保护静态内容；
 ...
 *             不可用时 m_data->m_platform 为 NULL，后续 API 安全退化。
 ...
 * @param      self 目标对象；可为 NULL。
 * @return     最近一次成功 resize 的尺寸；未 resize 返回 0×0。
 */
 ...
 * @param      self   目标对象；可为 NULL。
 * @param      region 静态内容区域集合；为 NULL/空表示清除。
 */
