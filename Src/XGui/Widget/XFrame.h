...
/** @brief XFrame 类虚函数表初始化（重载 Paint/Change/Event/Copy/Move）。 */
...
/** @brief 通过 XClass 虚表释放 XFrame 资源（栈/外部存储对象使用）。 */
...
/** @brief 查询框架形状（frameStyle 的 Shape 掩码，对标 frameShape()）。 */
...
/** @brief 查询框架阴影（frameStyle 的 Shadow 掩码，对标 frameShadow()）。 */
...
/** @brief 查询外形矩形（contentsRect 按四周边框宽度外扩，对标 frameRect()）。 */
 ...
 *             rect 参数可传 NULL/无效矩形，此时使用整个控件矩形。
 */
void XFrame_setFrameRect(XFrame* self, const XRect* rect);

/** @brief 框架尺寸提示（HLine=(-1,3)、VLine=(3,-1)、其余=基类，对标 sizeHint()）。 */
