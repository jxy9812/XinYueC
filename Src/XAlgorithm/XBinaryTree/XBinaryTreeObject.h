...
//数据
#define XBTreeNode_SetData(this_root,value) XBTreeNode_setData(this_root,&value)//二叉树-插入数据
#define XBTreeNode_GetData(this_root,Type) (*((Type*)(XBTreeNode_getData(this_root))))//二叉树-获取数据(继承的子类均可以使用)
//数据释放方法
typedef void (*ValueDeleteMethod)(void* value,void* args);
//定义节点类型
enum XBTreeNodeType
{
