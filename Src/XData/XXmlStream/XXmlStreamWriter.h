 ...
 * @details    提供文档、元素、属性、文本、DTD 和命名空间输出；设备访问只通过
 *             XinYueC 的 XIODevice 抽象完成，不调用平台 API。
 */
...
typedef struct XXmlStreamWriter
{
    ...
    XByteArray* m_externalBuffer; /**< 外部 QByteArray 输出目标，只借用 */
    XString*   m_externalString;  /**< 外部 QString 输出目标，只借用 */
