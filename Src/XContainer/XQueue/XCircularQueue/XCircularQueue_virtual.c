...
static bool VXCircularQueue_push_move(XCircularQueue* this_queue, void* pvValue);
//出队
...
XVtable* XCircularQueue_class_init()
{
	...
	void* table[] = { VXCircularQueue_push,VXCircularQueue_push_move,VXCircularQueue_pop,VXCircularQueue_top,VXCircularQueue_receive,VXCircularQueue_isFull };
	//追加虚函数
...
bool VXCircularQueue_push(XCircularQueue* this_queue, void* pvValue)
{
	...
	if (XContainerDataCopyMethod(this_queue))
	{
		...
		XContainerDataCopyMethod(this_queue)(((char*)XContainerDataPtr(this_queue)) + this_queue->m_tail * XContainerTypeSize(this_queue), pvValue);
	}
	else
	{
...
bool VXCircularQueue_push_move(XCircularQueue* this_queue, void* pvValue)
{
	if (VXCircularQueue_isFull(this_queue))
	{
		if (!this_queue->m_autoExpansion)
	...
	{
		memset(((char*)XContainerDataPtr(this_queue)) + this_queue->m_tail * XContainerTypeSize(this_queue), 0, XContainerTypeSize(this_queue));
		XContainerDataMoveMethod(this_queue)(((char*)XContainerDataPtr(this_queue)) + this_queue->m_tail * XContainerTypeSize(this_queue), pvValue);
	}
	else
	{
		memcpy(((char*)XContainerDataPtr(this_queue)) + this_queue->m_tail * XContainerTypeSize(this_queue), pvValue, XContainerTypeSize(this_queue));
