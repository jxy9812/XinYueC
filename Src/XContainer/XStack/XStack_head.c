...
void* StacketEnlargeCapacity(XSTACK* this_stack)
{
	...
	{
		void* _data = realloc(this_stack->object._data, this_stack->object._capacity * this_stack->object._typeSize * 2);
		if (_data == NULL)
		{
			perror("扩容失败sttor");
			exit(-1);
	...
	char* str1 = (char*)this_stack->object._data + this_stack->object._typeSize * this_stack->object._size;
