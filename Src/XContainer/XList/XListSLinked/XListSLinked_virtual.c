...
//Qt 6.8 对齐新增
static size_t VXList_remove_all(XListSLinked* this_list, const void* pvData);
static bool VXList_remove_one(XListSLinked* this_list, const void* pvData);
static bool VXList_remove_one(XListSLinked* this_list, const void* pvData);
static bool VXList_indexOf(const XListSLinked* this_list, const void* findVal, size_t from, XListSLinked_iterator* it);
static bool VXList_lastIndexOf(const XListSLinked* this_list, const void* findVal, size_t from, XListSLinked_iterator* it);
static size_t VXList_removeIf(XListSLinked* this_list, bool (*predicate)(const void* elemData, void* userData), void* userData);

XVtable* XListSLinked_class_init()
{
    XVTABLE_INIT_DEFAULT_SIZE(XLISTSLINKED_VTABLE_SIZE)
	XCLASS_SET_CLASS_NAME_DEFAULT("XListSLinked");
        //继承类
    ...
    XCLASS_SHOW_SIZE_DEFAULT(XListSLinked);
    return XVTABLE_DEFAULT;
}
#define CreatNode(this_list)    XContainer_malloc(this_list, ALIGN_UP(sizeof(XListSNode)+XContainerTypeSize(this_list),sizeof(void*)))
//#define CreatNode(this_list)   (XContainer_malloc(this_list, sizeof(XListSNode)+XContainerTypeSize(this_list)))
...
static bool ensureSharedData(XListSLinked* this_list)
{
    if (XContainerIsCow(this_list)) {
        if (!(XSharedData*)XContainerDataPtr(this_list)) {
            XSharedData* sd = XSharedData_create_ex(NULL, sizeof(XListSNode*), XContainer_memory(this_list));
            if (!sd) return false;
...
static bool VXListSLinkedDetachIfNeeded(XListSLinked* this_list)
{
    ...
    if (!(XSharedData*)XContainerDataPtr(this_list) || !XSharedData_isShared((XSharedData*)XContainerDataPtr(this_list)))
    ...
    XSharedData* newShared = XSharedData_create_ex(NULL, sizeof(XListSNode*), XContainer_memory(this_list));
    ...
    while (oldNode)
    {
        XListSNode* newNode = XContainer_malloc(this_list, ALIGN_UP(sizeof(XListSNode) + typeSize, sizeof(void*)));
        if (!newNode)
        {
            // 释放已创建的节点
            XListSNode* tmp = newHead;
            while (tmp) { XListSNode* next = tmp->next; XContainer_free(this_list, tmp); tmp = next; }
...
static void VXListSLinkedDataDelete(void* data, XListSLinked* this_list)
{
    ...
    while (head)
    {
        ...
        XContainer_free(this_list, head);
        head = next;
    }
...
static void removeNode(XListSLinked* this_list, XListSNode* prev, XListSNode* removeNode)
{
    ...
    XContainer_free(this_list, removeNode);
    //更新数量
    --XContainerSize(this_list);
    --XContainerCapacity(this_list);
}
...
void VXList_clear(XListSLinked* this_list)
{
    ...
    while (node)
    {
        ...
        XContainer_free(this_list, prev);
    }
    this_list->m_tail = NULL;
...
{
    XListSNode** head_ptr = XListSLinked_head_ptr(this_list);
    if (head_ptr && *head_ptr)
        return XListSNode_DataPtr(*head_ptr);
    return NULL;
}
...
// 单向链表的一次快排（分区函数）
static XListSNode* List_OneSort(XListSLinked* this_list, XListSNode* left, XListSNode* right, size_t typeSize, XCompare compare, XSortOrder order)
{
    if (left == NULL || right == NULL || left == right)
        return left;

    void* pivot = XContainer_malloc(this_list, typeSize);
    ...
    while (j != NULL) {
        ...
        if (((cmp == XCompare_Less) && (order == XSORT_ASC) || (cmp == XCompare_Equality) || (cmp == XCompare_Greater) && (order == XSORT_DESC)))//排序比较函数
        {
            ...
            void* temp = XContainer_malloc(this_list, typeSize);
    ...
    XContainer_free(this_list, temp);
    XContainer_free(this_list, pivot);
...
void VXList_sort(XListSLinked* this_list, XSortOrder order)
{
    ...
    // 使用现有的XStack
    XStack* stack = XStack_create_ex(XContainer_memory_type(this_list), sizeof(XListSNode*));
    if (stack == NULL)
        return;
    ...
    while (!XStack_isEmpty_base(stack)) {
        ...
        // 执行一次快排
        XListSNode* pivot = List_OneSort(this_list, h, t, XContainerTypeSize(this_list), XContainerCompare(this_list),order);
...
void VXClass_copy(XListSLinked* object, const XListSLinked* src)
{
        ...
        if (XContainerIsCow(object))
        {
            if ((XSharedData*)XContainerDataPtr(object))
                XSharedData_release_with((XSharedData*)XContainerDataPtr(object),
                    VXListSLinkedDataDelete, object, XContainer_memory(object));
        }
        else
        {
    ...
    else
    {
        ...
        while (srcNode)
        {
            XListSNode* newNode = XContainer_malloc(object, ALIGN_UP(sizeof(XListSNode) + typeSize, sizeof(void*)));
            if (!newNode)
            {
                while (newHead)
                {
    ...
    if (XContainerIsCow(this_list)) {
        if ((XSharedData*)XContainerDataPtr(this_list))
            XSharedData_release_with((XSharedData*)XContainerDataPtr(this_list),
                VXListSLinkedDataDelete, this_list, XContainer_memory(this_list));
    }
    else {
...
static bool VXList_indexOf(const XListSLinked* this_list, const void* findVal, size_t from, XListSLinked_iterator* it)
{
	XListSLinked_iterator tmpIt = XListSLinked_begin((XListSLinked*)this_list);
	if (!it) it = &tmpIt;
	size_t idx = 0;
	while (!XListSLinked_iterator_isEnd(&tmpIt)) {
		if (idx >= from &&
			XContainerCompare(this_list)(XListSLinked_iterator_data(&tmpIt), findVal) == XCompare_Equality) {
			*it = tmpIt;
			return true;
		}
		XListSLinked_iterator_add((XListSLinked*)this_list, &tmpIt);
		idx++;
	}
	return false;
}
...
static bool VXList_lastIndexOf(const XListSLinked* this_list, const void* findVal, size_t from, XListSLinked_iterator* it)
{
	bool found = false;
	XListSLinked_iterator tmpIt = XListSLinked_begin((XListSLinked*)this_list);
	XListSLinked_iterator lastFound;
	lastFound.node = NULL;
	size_t idx = 0;
	while (!XListSLinked_iterator_isEnd(&tmpIt)) {
...
static size_t VXList_removeIf(XListSLinked* this_list, bool (*predicate)(const void* elemData, void* userData), void* userData)
{
	if (!ensureSharedData(this_list) || !VXListSLinkedDetachIfNeeded(this_list))
		return 0;
	size_t removed = 0;
	XListSNode* node = *XListSLinked_head_ptr(this_list);
	XListSNode* prev = NULL;
	while (node) {
		XListSNode* next = node->next;
		if (predicate(XListSNode_DataPtr(node), userData)) {
			removeNode(this_list, prev, node);
			removed++;
			node = prev ? prev->next : *XListSLinked_head_ptr(this_list);
		} else {
			prev = node;
			node = next;
		}
...
#endif
/*           Qt 6.8 对齐：count / removeAll / removeOne / equals               */
/* ========================================================================== */
static size_t VXList_remove_all(XListSLinked* this_list, const void* pvData)
{
