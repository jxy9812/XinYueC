...
bool VXList_insert(XListSLinked* this_list, XListSNode* curNode, void* pvData)
{
    if (curNode == NULL)
    {
...
size_t VXList_insert_array(XListSLinked* this_list, XListSNode* curNode, const void* array, size_t count)
{
    //遍历节点
    XListSNode* prev = NULL;//前一个节点
    XListSNode* node = XContainerDataPtr(this_list);//当前节点
