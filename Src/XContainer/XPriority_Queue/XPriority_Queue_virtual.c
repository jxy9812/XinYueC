...
void XPriority_Queue_class_init()
{
	...
	//追加函数
	XVtable_append_array(XPriority_QueueVtable, vtable, sizeof(vtable) / sizeof(vtable[0]));
}
//插入向上调整
...
void VXPriority_Queue_push(XPriority_Queue* this_queue, void* LpValue)
{
	...
	size_t size = ContainerSize(this_queue) - 1;
	if (size > 0)//一个元素不用调整
		AdjustUp(ContainerDataPtr(this_queue), ContainerTypeSize(this_queue), size, this_queue->compare);
}
