...
{
	XVTABLE_INIT_DEFAULT_SIZE(XPWMDEVICE_VTABLE_SIZE)
	XCLASS_SET_CLASS_NAME_DEFAULT("XPWMDeviceBase");
	//继承类
	...
	XCLASS_SHOW_SIZE_DEFAULT(XPWMDeviceBase);
...
void VXPWMDevice_start(XPWMDeviceBase* pwm)
{
	if (pwm != NULL)
	{
		...
		{
			//XObject_postEvent(pwm, XEventFunc_create(pwm->m_runChangeCallback, pwm, NULL), XEVENT_PRIORITY_NORMAL);
		}
	}
}
