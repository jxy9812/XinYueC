                       ...
                       int argc, const char* const* argv, void* userData)
{
    ...
    for (i = 0; i < shell->m_historyCount; ++i) {
...
static const XConsoleCommand g_historyCommand = {
    "history", NULL, "列出历史命令", "history", 0, 0,
...
static const XConsoleCommand g_helpCommand = {
    ...
    XConsoleCommandFlag_None, xcs_help, NULL, 0, NULL
};
                    ...
                    int argc, const char* const* argv, void* userData)
{
    ...
    (void)userData;
    for (i = 0; i < argc; ++i) {
        if (i && !xcs_write_literal(shell, " ")) return XConsoleResult_IoError;
        if (!xcs_write_literal(shell, argv[i])) return XConsoleResult_IoError;
    }
    return xcs_write_literal(shell, "\n") ? XConsoleResult_Ok : XConsoleResult_IoError;
}

static const XConsoleCommand g_versionCommand = {
    "version", NULL, "显示库版本", "version", 0, 0,
    XConsoleCommandFlag_None, xcs_version, NULL, 0, NULL
};
static const XConsoleCommand g_echoCommand = {
    "echo", NULL, "回显参数", "echo <text...>", 0, -1,
...
static const XConsoleCommand g_clearCommand = {
...
#if XCONSOLE_SHELL_LINE_EDITOR_ON
    registered = xcs_register_static_commands(self, &g_clearCommand, 1) && registered;
#endif
#if XCONSOLE_SHELL_HISTORY_ON
...
{
    if (self) self->m_running = running;
}
                              ...
                              const char* detail)
{
    ...
    if (!detail) {
        const char* fallback = result == XConsoleResult_UnknownCommand ? "unknown command" :
                               result == XConsoleResult_InvalidArgument ? "invalid argument" :
                               result == XConsoleResult_PermissionDenied ? "permission denied" :
                               result == XConsoleResult_Cancelled ? "cancelled" : "failed";
