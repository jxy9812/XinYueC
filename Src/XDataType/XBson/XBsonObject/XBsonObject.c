...
XBsonObject* XBsonObject_create() {
    XBsonObject* object = (XBsonObject*)XMemory_malloc(sizeof(XBsonObject));
    if (object) {
        XBsonObject_init(object);
    }
    return object;
}
...
XBsonObject* XBsonObject_create_copy(const XBsonObject* other) {
    ...
    XBsonObject* object = XBsonObject_create();
    if (object) {
        XBsonObject_copy_base(object, other);
    }
    return object;
}
...
XBsonObject* XBsonObject_create_move(XBsonObject* other) {
    ...
    XBsonObject* object = XBsonObject_create();
    if (object) {
        XBsonObject_move_base(object, other);
    }
    return object;
}
...
void XBsonObject_from_json_object(XBsonObject* bson_obj, const XJsonObject* json_obj) {
   ...
   /* XBsonObject_clear(bson_obj);
    ...
    while (!XMapIterator_isEnd(&it)) {
        ...
        if (bson_val) {
            XBsonObject_insert_move(bson_obj, XString_toUtf8(key), bson_val);
        }
...
bool XBsonObject_from_bytes(XBsonObject* object, const uint8_t* data, size_t size) {
    ...
    //// 使用XStack处理递归
    //XStack* stack = XStack_create(sizeof(XBsonObject*));
    ...
    //    XBsonObject* current_obj = *(XBsonObject**)XStack_top_base(stack);
    ...
    //    // 添加到当前对象
    //    XBsonObject_insert_move(current_obj, XString_toUtf8(key), value);
    //    XString_delete_base(key);
