...
static void XVectorPrintInt(X* v, const char* prefix)
{
	XPrintf("%s", prefix);
	X_for_each(v, XFor_each_int, NULL);
	XPrintf("\n");
}

static X* XVectorMakeInt(const int* arr, size_t n)
{
	X* v = XVector_Create(int);
	XContainerSetCompare(v, int_compare);
	for (size_t i = 0; i < n; i++)
		X_Push_Back_Base(v, int, arr[i]);
	return v;
}
...
static void XVectorCreateTest(void)
{
	XPrintf("===== 创建与初始化测试 =====\n");
	{
		...
		XPrintf("XVector_Create(int): size=%zu, capacity=%zu, isEmpty=%s\n",
			X_size_base(v), X_capacity_base(v), X_isEmpty_base(v) ? "是" : "否");
		X_delete_base(v);
	}
	{
		X* v = XVector_create_ex(sizeof(int), false);
		XPrintf("create_ex(int,cow=false): size=%zu, typeSize=%zu\n",
			X_size_base(v), X_typeSize_base(v));
		X_delete_base(v);
	}
	{
		int arr[] = { 1, 2, 3, 4, 5 };
		X* src = XVectorMakeInt(arr, 5);
		X* copy = XVector_create_copy(src);
		XPrintf("create_copy: equals=%s\n", X_equals(src, copy) ? "是" : "否");
		X_Push_Back_Base(src, int, 99);
	...
	{
		int arr[] = { 10, 20, 30 };
		X* src = XVectorMakeInt(arr, 3);
		X* moved = XVector_create_move(src);
		XPrintf("create_move: moved.size=%zu, src.isEmpty=%s\n",
			X_size_base(moved), X_isEmpty_base(src) ? "是" : "否");
	...
	{
		X v;
		X_init(&v, sizeof(int), true);
		XPrintf("init: size=%zu, isEmpty=%s\n",
			X_size_base(&v), X_isEmpty_base(&v) ? "是" : "否");
		X_deinit_base(&v);
	}
	XPrintf("\n");
}
...
static void XVectorCapacityTest(void)
{
	...
	X* v = XVector_Create(int);
	XContainerSetCompare(v, int_compare);
	XPrintf("初始: size=%zu, capacity=%zu\n", X_size_base(v), X_capacity_base(v));
	int arr[] = { 1, 2, 3, 4, 5 };
	for (size_t i = 0; i < 5; i++) X_Push_Back_Base(v, int, arr[i]);
	XPrintf("填充5个: size=%zu, capacity=%zu\n", X_size_base(v), X_capacity_base(v));
	X_reserve_base(v, 100);
	...
	X_resizeForOverwrite(v, 10);
	XPrintf("resizeForOverwrite(10): size=%zu\n", X_size_base(v));
	XPrintf("max_size=%zu, maxSize(sizeof(int))=%zu\n", X_max_size(v), X_maxSize(sizeof(int)));
	X_shrink_to_fit(v);
	XPrintf("shrink_to_fit: capacity=%zu\n", X_capacity_base(v));
	XPrintf("X_size=%zu, X_count_base=%zu, X_length_base=%zu\n", X_size(v), X_count_base(v), X_length_base(v));
	X_delete_base(v);
	XPrintf("\n");
}
...
static void XVectorAccessTest(void)
{
	...
	X* v = XVectorMakeInt(arr, 5);
	XPrintf("at(0)=%d, at(2)=%d, at(4)=%d\n", *(int*)X_at_base(v, 0), *(int*)X_at_base(v, 2), *(int*)X_at_base(v, 4));
	XPrintf("operator[](2)=%d\n", X_At_Base(v, 2, int));
	XPrintf("front=%d, back=%d\n", *(int*)X_front_base(v), *(int*)X_back_base(v));
	XPrintf("Front_Base=%d, Back_Base=%d\n", X_Front_Base(v, int), X_Back_Base(v, int));
	XPrintf("constFirst=%d, constLast=%d\n", *(int*)X_constFirst(v), *(int*)X_constLast(v));
	XPrintf("X_front=%d, X_back=%d\n", *(int*)X_front(v), *(int*)X_back(v));
	int def = -1;
	XPrintf("value(2)=%d, value(99,默认-1)=%d\n", *(int*)X_value(v, 2, NULL), *(int*)X_value(v, 99, &def));
	{
		X* f = X_first(v, 3);
	...
	int* d = (int*)X_data(v);
	const int* cd = (const int*)X_constData(v);
	XPrintf("data()[1]=%d, constData()[3]=%d\n", d[1], cd[3]);
	{
		X* empty = XVector_Create(int);
		XPrintf("空向量: front=%s, back=%s, at(0)=%s\n",
			X_front_base(empty) ? "非空" : "空", X_back_base(empty) ? "非空" : "空", X_at_base(empty, 0) ? "非空" : "空");
		X_delete_base(empty);
	}
	X_delete_base(v);
	XPrintf("\n");
...
static void XVectorFrontOpsTest(void)
{
	...
	{
		int a[] = { 100 };
		X* v3 = XVectorMakeInt(a, 1);
		X_push_front_move_3(v, v3);
		XVectorPrintInt(v, "push_front_move_3({100}): ");
		X_delete_base(v3);
	}
	int prep[] = { 5, 6 };
	X_prepend_2(v, prep, 2);
...
static void XVectorInsertTest(void)
{
	...
	XVectorPrintInt(v, "初始: ");
	int head[] = { 0 };
	X_insert_1_base(v, 0, head, 1);
	XVectorPrintInt(v, "insert_1(0,{0}): ");
	int mid[] = { 2, 3, 4 };
	X_insert_1_base(v, 2, mid, 3);
...
static void XVectorRemoveTest(void)
{
	XPrintf("===== 删除操作测试 =====\n");
	{
		...
		X* v = XVectorMakeInt(arr, 8);
		XVectorPrintInt(v, "初始: ");
		int t = 2;
		size_t r = X_removeAll(v, &t);
	...
	}
	{
		...
		X* v = XVectorMakeInt(arr, 10);
		size_t r = X_removeIf(v, XVectorRemoveEven, NULL);
	...
	}
	{
		...
		X* v = XVectorMakeInt(arr, 7);
		for (X it = X_begin(v), endIt = X_end(v); !X_equality(&it, &endIt);)
		{
			if (*(int*)X_data(&it) == 2) X_erase_base(v, &it, &it);
			else X_add(&it);
		}
	...
	}
	{
		...
		X* v = XVectorMakeInt(arr, 5);
		X_clear_base(v);
		XPrintf("clear: isEmpty=%s, size=%zu\n", X_isEmpty_base(v) ? "是" : "否", X_size_base(v));
		X_delete_base(v);
	}
	XPrintf("\n");
}
...
static void XVectorTakeTest(void)
{
	...
	XVectorPrintInt(v, "初始: ");
	{
		int* p = (int*)X_takeAt(v, 2);
		XPrintf("takeAt(2)=%d\n", *p); XFree_System(p);
	}
	XVectorPrintInt(v, "  takeAt后: ");
	{
		int* p = (int*)X_takeFirst(v);
		XPrintf("takeFirst=%d\n", *p); XFree_System(p);
	}
	XVectorPrintInt(v, "  takeFirst后: ");
	{
		int* p = (int*)X_takeLast(v);
		XPrintf("takeLast=%d\n", *p); XFree_System(p);
	}
	XVectorPrintInt(v, "  takeLast后: ");
	{
		...
		XPrintf("空: takeAt=%s, takeFirst=%s, takeLast=%s\n",
			X_takeAt(empty, 0) ? "非空" : "空", X_takeFirst(empty) ? "非空" : "空", X_takeLast(empty) ? "非空" : "空");
		X_delete_base(empty);
	}
	X_delete_base(v);
	XPrintf("\n");
...
static void XVectorReplaceTest(void)
{
	...
	X_replace_move_1(v, 3, p);
	XVectorPrintInt(v, "replace_move_1(3,300): ");
	int av = 888;
	X_replace(v, 1, &av);
	XVectorPrintInt(v, "X_replace(1,888): ");
	X_delete_base(v);
	XPrintf("\n");
}
...
static void XVectorSortFindTest(void)
{
	XPrintf("===== 排序与查找测试 =====\n");
	{
		...
		X* v = XVectorMakeInt(arr, 10);
		X_sort_base(v, XSORT_ASC);
		XVectorPrintInt(v, "升序: ");
		X_sort_base(v, XSORT_DESC);
	...
	}
	{
		...
		X* v = XVectorMakeInt(arr, 7);
		int f = 20, n = 999;
		XPrintf("indexOf(20,0)=%lld, indexOf(20,3)=%lld\n", (long long)X_indexOf(v, &f, 0), (long long)X_indexOf(v, &f, 3));
		XPrintf("lastIndexOf(20,-1)=%lld, lastIndexOf(20,4)=%lld\n", (long long)X_lastIndexOf(v, &f, -1), (long long)X_lastIndexOf(v, &f, 4));
		XPrintf("indexOf(999)=%lld (期望-1)\n", (long long)X_indexOf(v, &n, 0));
		XPrintf("contains(20)=%s, contains(999)=%s\n", X_contains(v, &f) ? "是" : "否", X_contains(v, &n) ? "是" : "否");
		int fst = 10, lst = 50, wr = 99;
		XPrintf("startsWith(10)=%s, startsWith(99)=%s\n", X_startsWith(v, &fst) ? "是" : "否", X_startsWith(v, &wr) ? "是" : "否");
		XPrintf("endsWith(50)=%s, endsWith(99)=%s\n", X_endsWith(v, &lst) ? "是" : "否", X_endsWith(v, &wr) ? "是" : "否");
		XPrintf("count_value(20)=%zu, count_value(999)=%zu\n", X_count_value(v, &f), X_count_value(v, &n));
		XPrintf("X_count(20)=%zu\n", X_count(v, &f));
		X_delete_base(v);
	}
	XPrintf("\n");
}
...
static void XVectorCompareTest(void)
{
	...
	int a3[] = { 1, 2, 3, 4, 6 };
	int a4[] = { 1, 2, 3 };
	X* v1 = XVectorMakeInt(a1, 5);
	X* v2 = XVectorMakeInt(a2, 5);
	X* v3 = XVectorMakeInt(a3, 5);
	X* v4 = XVectorMakeInt(a4, 3);
	XPrintf("v1==v2: equals=%s, compare=%d\n", X_equals(v1, v2) ? "是" : "否", X_compare(v1, v2));
	XPrintf("v1<v3: lessThan=%s, compare=%d\n", X_lessThan(v1, v3) ? "是" : "否", X_compare(v1, v3));
	XPrintf("v1>v4: greaterThan=%s\n", X_greaterThan(v1, v4) ? "是" : "否");
	XPrintf("v1>=v2: greaterEqual=%s\n", X_greaterEqual(v1, v2) ? "是" : "否");
	XPrintf("v1<=v2: lessEqual=%s\n", X_lessEqual(v1, v2) ? "是" : "否");
	X_delete_base(v1); X_delete_base(v2); X_delete_base(v3); X_delete_base(v4);
	XPrintf("\n");
}
...
static void XVectorCowTest(void)
{
	...
	int arr[] = { 1, 2, 3, 4, 5 };
	X* v = XVectorMakeInt(arr, 5);
	X* copy = XVector_create_copy(v);
	XPrintf("拷贝后: isSharedWith=%s, isDetached=%s\n",
		X_isSharedWith(v, copy) ? "是" : "否", X_isDetached(v) ? "是" : "否");
	X_detach(v);
	XPrintf("detach后: isSharedWith=%s, isDetached=%s\n",
		X_isSharedWith(v, copy) ? "是" : "否", X_isDetached(v) ? "是" : "否");
	X_Push_Back_Base(copy, int, 99);
	XPrintf("修改copy后: isSharedWith=%s\n", X_isSharedWith(v, copy) ? "是" : "否");
	XPrintf("NULL: isSharedWith(v,NULL)=%s, isDetached(NULL)=%s\n",
		X_isSharedWith(v, NULL) ? "是" : "否", X_isDetached(NULL) ? "是" : "否");
	X_delete_base(v); X_delete_base(copy);
	XPrintf("\n");
}
...
static void XVectorSliceTest(void)
{
	...
	{
		X* m = X_mid(v, 2, 4);
		XVectorPrintInt(m, "mid(2,4): "); X_delete_base(m);
	}
	{
		X* m = X_mid(v, 5, -1);
		XVectorPrintInt(m, "mid(5,-1): "); X_delete_base(m);
	}
	{
		X* f = X_first(v, 3);
		XVectorPrintInt(f, "first(3): "); X_delete_base(f);
	}
	{
		X* l = X_last(v, 3);
		XVectorPrintInt(l, "last(3): "); X_delete_base(l);
	}
	{
		X* s = X_sliced_1(v, 7);
		XVectorPrintInt(s, "sliced(7): "); X_delete_base(s);
	}
	{
		X* s = X_sliced_2(v, 3, 3);
		XVectorPrintInt(s, "sliced(3,3): "); X_delete_base(s);
	}
	X_delete_base(v);
	XPrintf("\n");
}
...
static void XVectorMoveSwapTest(void)
{
	...
	}
	{
		...
		X* v1 = XVectorMakeInt(a1, 3);
		X* v2 = XVectorMakeInt(a2, 4);
		X_swap_base(v1, v2);
		XVectorPrintInt(v1, "swap后v1: ");
		XVectorPrintInt(v2, "swap后v2: ");
		XPrintf("v1.size=%zu, v2.size=%zu\n", X_size_base(v1), X_size_base(v2));
		X_delete_base(v1); X_delete_base(v2);
	}
	XPrintf("\n");
}
...
static void XVectorSafetyTest(void)
{
	...
	XPrintf("insert_2(NULL)=%s, push_front_2(NULL)=%s\n",
		X_insert_2(NULL, 0, &val) ? "是" : "否", X_push_front_2(NULL, &val, 1) ? "是" : "否");
	XPrintf("insert_move_2(NULL)=%s, contains(NULL)=%s\n",
		X_insert_move_2(NULL, 0, &val) ? "是" : "否", X_contains(NULL, &val) ? "是" : "否");
	XPrintf("first(NULL)=%s, mid(NULL)=%s\n",
		X_first(NULL, 3) ? "非空" : "空", X_mid(NULL, 0, 1) ? "非空" : "空");
	XPrintf("isEmpty(NULL)=%s, size(NULL)=%zu\n", X_isEmpty_base(NULL) ? "是" : "否", X_size_base(NULL));
	XPrintf("indexOf(NULL)=%lld, lastIndexOf(NULL)=%lld\n",
		(long long)X_indexOf(NULL, &val, 0), (long long)X_lastIndexOf(NULL, &val, -1));
	{
		int arr[] = { 1, 2, 3 };
		X* v = XVectorMakeInt(arr, 3);
		X* dbl = XVector_create(sizeof(double));
		X_Push_Back_Base(dbl, double, 1.5);
		XPrintf("insert_3(类型不一致)=%s, insert_move_3(类型不一致)=%s（均应否）\n",
			X_insert_3(v, 0, dbl) ? "是" : "否", X_insert_move_3(v, 0, dbl) ? "是" : "否");
		X_delete_base(v); X_delete_base(dbl);
	}
	{
		int arr[] = { 1, 2, 3 };
		X* v = XVectorMakeInt(arr, 3);
		X* dbl = XVector_create(sizeof(double));
		X_Push_Back_Base(dbl, double, 3.14);
		XPrintf("push_front_3(类型不一致)=%s, push_back_3(类型不一致)=%s（均应否）\n",
			X_push_front_3(v, dbl) ? "是" : "否", X_push_back_3(v, dbl) ? "是" : "否");
		X_delete_base(v); X_delete_base(dbl);
	}
	XPrintf("\n");
}
...
void XVectorAllTest(void)
{
	...
	XVectorFillTest();
	XVectorSafetyTest();
	XPrintf("========== XVector 全部测试结束 ==========\n");
	XCoreApplication_quit();
}

void XMenu_XTest(XMenu* root)
{
	XMenu* menu = XMenu_create("X(数组)");
