...
static void XFuncCodeCb0x30(uint8_t code, void* obj, void* data, void* userData)
{
	...
	if (cJSON_IsString(cuttingMotorSpeedDocking) && (cuttingMotorSpeedDocking->valuestring != NULL)) {
		...
		if (str != NULL)
		{
			XStringList* strlist = XString_split_utf8(str, "-",XCharCaseInsensitive);
			if (strlist != NULL )
			{
				if (XContainerSize(strlist) == 3)
				{
					printf("%lf %lf %lf\n"
						, XString_toDouble(XStringList_at_base(strlist, 0),NULL)
						, XString_toDouble(XStringList_at_base(strlist, 1), NULL)
						, XString_toDouble(XStringList_at_base(strlist, 2), NULL)
					);
				}
