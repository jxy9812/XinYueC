...
{
    int pass = 0, fail = 0;
    XPrintf("========== XModbusCommEvent 单元测试开始 ==========\n");
    ...
    // 1. 测试 create 和默认值
    {
        ...
        if (byte == 0x00) {
            XPrintf("  [通过] create(InitiatedCommunicationRestart) = 0x%02X\n", byte);
            pass++;
        } else {
            XPrintf("  [失败] create(InitiatedCommunicationRestart) = 0x%02X, 期望 0x00\n", byte);
            fail++;
        }
    }
    ...
    // 2. 测试 fromUint8 和 toUint8
    {
        ...
        if (result == testByte) {
            XPrintf("  [通过] fromUint8/toUint8 往返: 0x%02X\n", result);
            pass++;
        } else {
            XPrintf("  [失败] fromUint8/toUint8: 0x%02X, 期望 0x%02X\n", result, testByte);
            fail++;
        }
    }
    ...
    // 3. 测试 NULL 指针保护
    {
        ...
        if (ok) {
            XPrintf("  [通过] NULL 指针保护全部正确\n");
            pass++;
        } else {
            XPrintf("  [失败] NULL 指针保护异常\n");
            fail++;
        }
    }
    ...
    // 4. 测试 setEventByte
    {
        ...
        if (XModbusCommEvent_toUint8(&e) == 0x40) {
            XPrintf("  [通过] setEventByte(SentEvent) = 0x%02X\n", XModbusCommEvent_toUint8(&e));
            pass++;
        } else {
            XPrintf("  [失败] setEventByte(SentEvent) = 0x%02X, 期望 0x40\n", XModbusCommEvent_toUint8(&e));
            fail++;
        }
    }
    ...
    // 5. 测试 orWithSendFlag 和 testSendFlag
    {
        ...
        if (byte == 0x41 &&
            ...
            XPrintf("  [通过] orWithSendFlag(ReadException) = 0x%02X, testSendFlag=true, isSent=true\n", byte);
            pass++;
        } else {
            XPrintf("  [失败] orWithSendFlag(ReadException) = 0x%02X, 期望 0x41\n", byte);
            fail++;
        }
    }
    ...
    // 6. 测试 orWithReceiveFlag 和 testReceiveFlag
    {
        ...
        if (byte == 0xC0 &&
            ...
            XPrintf("  [通过] orWithReceiveFlag(Broadcast) = 0x%02X, testReceiveFlag=true, isReceive=true\n", byte);
            pass++;
        } else {
            XPrintf("  [失败] orWithReceiveFlag(Broadcast) = 0x%02X, 期望 0xC0\n", byte);
            fail++;
        }
    }
    ...
    // 7. 测试 isListenOnlyMode
    {
        ...
        if (XModbusCommEvent_isListenOnlyMode(&e)) {
            XPrintf("  [通过] isListenOnlyMode(EnteredListenOnlyMode) = true\n");
            pass++;
        } else {
            XPrintf("  [失败] isListenOnlyMode(EnteredListenOnlyMode) = false\n");
            fail++;
        }
    }
    ...
    // 8. 测试 isRestartEvent
    {
        ...
        if (XModbusCommEvent_isRestartEvent(&e)) {
            XPrintf("  [通过] isRestartEvent(InitiatedCommunicationRestart) = true\n");
            pass++;
        } else {
            XPrintf("  [失败] isRestartEvent(InitiatedCommunicationRestart) = false\n");
            fail++;
        }
    }
    ...
    // 9. 测试 combineEventByte
    {
        ...
        if ((uint8_t)result == 0x42) {
            XPrintf("  [通过] combineEventByte(SentEvent, ServerAbort) = 0x%02X\n", (uint8_t)result);
            pass++;
        } else {
            XPrintf("  [失败] combineEventByte(SentEvent, ServerAbort) = 0x%02X, 期望 0x42\n", (uint8_t)result);
            fail++;
        }
    }
    ...
    // 10. 测试 combineEventByteWithReceive
    {
        ...
        if ((uint8_t)result == 0x82) {
            XPrintf("  [通过] combineEventByteWithReceive(ReceiveEvent, CommError) = 0x%02X\n", (uint8_t)result);
            pass++;
        } else {
            XPrintf("  [失败] combineEventByteWithReceive(ReceiveEvent, CommError) = 0x%02X, 期望 0x82\n", (uint8_t)result);
            fail++;
        }
    }
    ...
    // 11. 测试多标志组合
    {
        ...
        if (byte == 0x92 &&
            ...
            !XModbusCommEvent_testReceiveFlag(&e, XModbusCommEvent_BroadcastReceived) &&
            XModbusCommEvent_isReceiveEvent(&e)) {
            XPrintf("  [通过] 多标志组合: 0x%02X, 所有位测试正确\n", byte);
            pass++;
        } else {
            XPrintf("  [失败] 多标志组合: 0x%02X, 期望 0x92\n", byte);
            fail++;
        }
    }
    ...
    // 12. 测试 setEventByte 重置事件
    {
        ...
        if (XModbusCommEvent_isRestartEvent(&e) && XModbusCommEvent_toUint8(&e) == 0x00) {
            XPrintf("  [通过] setEventByte 重置为 InitiatedCommunicationRestart = 0x00\n");
            pass++;
        } else {
            XPrintf("  [失败] setEventByte 重置后 = 0x%02X, 期望 0x00\n", XModbusCommEvent_toUint8(&e));
            fail++;
        }
    }
    ...
    // 13. 测试 setEventByte 对 NULL 的保护
    {
        ...
        XPrintf("  [通过] setEventByte(NULL) 不崩溃\n");
        pass++;
    }
    ...
    // 14. 测试 orWithSendFlag 对 NULL 的保护
    {
        ...
        if (result == NULL) {
            XPrintf("  [通过] orWithSendFlag(NULL) 返回 NULL\n");
            pass++;
        } else {
            XPrintf("  [失败] orWithSendFlag(NULL) 返回非 NULL\n");
            fail++;
        }
    }
...
void XModbusAduTest(void)
{
    ...
    // ========== 1. 测试 init/deinit ==========
    {
        ...
        if (adu.m_rawData == NULL && adu.m_data == NULL && adu.m_serverAddress == 0xFF) {
            XPrintf("  [通过] init 初始化正确\n");
            pass++;
        } else {
            XPrintf("  [失败] init 初始化失败\n");
            fail++;
        }
        XModbusAdu_deinit(&adu);
        if (adu.m_rawData == NULL && adu.m_data == NULL) {
            XPrintf("  [通过] deinit 释放正确\n");
            pass++;
        } else {
            XPrintf("  [失败] deinit 释放失败\n");
            fail++;
        }
    }
    ...
    // ========== 4. 测试 RTU 帧格式 ==========
    {
        ...
        if (!frame) {
            XPrintf("  [失败] createRtuFrame 返回 NULL\n");
            fail++;
        } else {
            ...
            if (frameSize == 8 && raw[0] == 0x01 && raw[1] == 0x03 &&
                ...
                XPrintf("  [通过] createRtuFrame 格式正确\n");
                pass++;
            } else {
                XPrintf("  [失败] createRtuFrame 格式错误: 大小=%zu, [0]=0x%02X, [1]=0x%02X\n",
            ...
            if (!adu) {
                ...
                if (ok) {
                    XPrintf("  [通过] createRtuFrame + parseRtu 完整正确\n");
                    pass++;
                } else {
                    XPrintf("  [失败] createRtuFrame + parseRtu 结果异常\n");
                    fail++;
                }
                XModbusAdu_delete(adu);
            }
    ...
    // ========== 5. 测试 ASCII 帧格式 ==========
    {
        ...
        if (!frame) {
            XPrintf("  [失败] createAsciiFrame 返回 NULL\n");
            fail++;
        } else {
            size_t frameSize = XByteArray_size_base(frame);
            uint8_t* raw = XByteArray_data(frame);
            ...
            if (frameSize >= 5 && raw[0] == ':' && raw[frameSize - 2] == '\r' && raw[frameSize - 1] == '\n') {
                XPrintf("  [通过] createAsciiFrame 格式正确\n");
                pass++;
            } else {
                XPrintf("  [失败] createAsciiFrame 格式错误: 大小=%zu, [0]=0x%02X\n", frameSize, raw[0]);
                fail++;
            }

            XModbusAdu* adu = XModbusAdu_parseAscii(raw, frameSize);
            if (!adu) {
                XPrintf("  [失败] parseAscii 返回 NULL\n");
                fail++;
            } else {
                bool ok = true;
                if (XModbusAdu_serverAddress(adu) != 1) ok = false;
                ...
                if (ok) {
                    XPrintf("  [通过] createAsciiFrame + parseAscii 完整正确\n");
                    pass++;
                } else {
                    XPrintf("  [失败] createAsciiFrame + parseAscii 结果异常\n");
                    fail++;
                }
                XModbusAdu_delete(adu);
            }
    ...
    // ========== 6. parseRtu 边界测试 ==========
    {
        ...
        if (adu == NULL) {
            XPrintf("  [通过] parseRtu 短数据返回 NULL\n");
            pass++;
        } else {
            XPrintf("  [失败] parseRtu 短数据未返回 NULL\n");
            fail++;
            XModbusAdu_delete(adu);
        }
        ...
        if (adu == NULL) {
            XPrintf("  [通过] parseRtu NULL 输入返回 NULL\n");
            pass++;
        } else {
            XPrintf("  [失败] parseRtu NULL 输入未返回 NULL\n");
            fail++;
            XModbusAdu_delete(adu);
        }
    }
    ...
    // ========== 7. parseAscii 边界测试 ==========
    {
        ...
        if (adu == NULL) {
            XPrintf("  [通过] parseAscii 无':'前缀返回 NULL\n");
            pass++;
        } else {
            XPrintf("  [失败] parseAscii 无':'前缀未返回 NULL\n");
            fail++;
            XModbusAdu_delete(adu);
        }
        ...
        if (adu == NULL) {
            XPrintf("  [通过] parseAscii 短数据返回 NULL\n");
            pass++;
        } else {
            XPrintf("  [失败] parseAscii 短数据未返回 NULL\n");
            fail++;
            XModbusAdu_delete(adu);
        }
        ...
        if (adu == NULL) {
            XPrintf("  [通过] parseAscii NULL 输入返回 NULL\n");
            pass++;
        } else {
            XPrintf("  [失败] parseAscii NULL 输入未返回 NULL\n");
            fail++;
            XModbusAdu_delete(adu);
        }
    }
    ...
    // ========== 8. 校验和测试 ==========
    {
        ...
        if (lrc == 0xF2) {
            XPrintf("  [通过] calculateLRC 正确\n");
            pass++;
        } else {
            XPrintf("  [失败] calculateLRC = 0x%02X, 期望 0xF2\n", lrc);
            fail++;
        }
    ...
    // ========== 9. 错误校验测试 ==========
    {
        ...
        if (adu && !XModbusAdu_matchingChecksum(adu)) {
            XPrintf("  [通过] 错误 CRC 校验正确检测\n");
            pass++;
        } else {
            XPrintf("  [失败] 错误 CRC 校验未检测\n");
            fail++;
        }
        ...
        if (frame2) {
            ...
            if (adu2) {
                ...
                if (!XModbusPdu_isException(&outPdu) &&
                    ...
                    XPrintf("  [通过] pdu() 提取 XModbusPdu 正确\n");
                    pass++;
                } else {
                    XPrintf("  [失败] pdu() 提取失败\n");
                    fail++;
    ...
    // ========== 11. createRtuFrame NULL PDU 测试 ==========
    {
        ...
        if (frame3 == NULL) {
            XPrintf("  [通过] createRtuFrame(NULL PDU) 返回 NULL\n");
            pass++;
        } else {
    ...
    // ========== 12. createAsciiFrame NULL PDU 测试 ==========
    {
        ...
        if (frame4 == NULL) {
            XPrintf("  [通过] createAsciiFrame(NULL PDU) 返回 NULL\n");
            pass++;
        } else {
    ...
    // ========== 13. 多地址循环测试 ==========
    {
        ...
        if (ok) {
            XPrintf("  [通过] 100 次循环 RTU/ASCII 均成功\n");
            pass++;
        } else {
