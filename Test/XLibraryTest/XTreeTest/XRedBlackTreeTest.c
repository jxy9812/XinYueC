...
void XRedBlackTreeTest()
{
	...
	int* LPa = a;
	XPrintf("插入测试\n");
	XRBTreeNode* root = XRBTree_insert(NULL, int_compare,XCompareRuleTwo_BinaryTree, LPa++, sizeof(int));
	for (size_t i = 0; i < sizeof(a) / sizeof(a[0]) - 1; i++)
	{
		if (i == 11)
			i = 11;
		XRBTree_insert(&root, int_compare, XCompareRuleTwo_BinaryTree, LPa++, sizeof(int));
	}
	if (root != NULL)
	{
		//中序测试
	...
	{
		XRBTreeNode* removeNode= XRBTree_remove(&root,int_compare,XCompareRuleOne_BinaryTree, a + i,sizeof(int));
