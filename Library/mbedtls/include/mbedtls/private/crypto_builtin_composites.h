...
typedef struct {
    ...
    /* Keep the PSA operation layout independent from the private build
     * switch. The XCryptographic path stores an owned pointer; the legacy
     * path retains its original hash/opad state. */
    union {
        void *MBEDTLS_PRIVATE(xcryptographic);
        struct {
            struct psa_hash_operation_s hash_ctx;
            uint8_t opad[PSA_HMAC_MAX_HASH_BLOCK_SIZE];
        } MBEDTLS_PRIVATE(legacy);
    } MBEDTLS_PRIVATE(ctx);
} mbedtls_psa_hmac_operation_t;

#define MBEDTLS_PSA_HMAC_OPERATION_INIT { 0, { 0 } }
...
typedef struct {
    ...
    /* XCryptographic ECJPAKE context; kept opaque so PSA operation layout
     * does not expose an XCryptographic implementation type. */
    void *MBEDTLS_PRIVATE(xcryptographic_jpake);
#endif
