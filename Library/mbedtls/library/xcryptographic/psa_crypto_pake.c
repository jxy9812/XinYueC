...
static psa_status_t psa_pake_ecjpake_setup(mbedtls_psa_pake_operation_t *operation)
{
    ...
    if (!operation->password || operation->password_len == 0) {
        return PSA_ERROR_INVALID_ARGUMENT;
    }
    role = operation->role == MBEDTLS_ECJPAKE_CLIENT ?
           XCryptographic_EcjPakeRole_Client : XCryptographic_EcjPakeRole_Server;
    operation->xcryptographic_jpake = XCryptographic_ecjPakeCreate(
        role, XByteArrayView_create_data(operation->password,
                                         (int64_t)operation->password_len));
    mbedtls_platform_zeroize(operation->password, operation->password_len);
    return operation->xcryptographic_jpake ? PSA_SUCCESS : PSA_ERROR_GENERIC_ERROR;
#else
    ...
    return PSA_SUCCESS;
#endif /* MBEDTLS_USE_XCRYPTOGRAPHIC_JPAKE */
}
#endif
    ...
    size_t *output_length)
{
    ...
    if (PSA_ALG_IS_JPAKE(operation->alg)) {
...
#if defined(MBEDTLS_USE_XCRYPTOGRAPHIC_JPAKE)
            XByteArrayView result = XCryptographic_ecjPakeWriteRoundOneInto(
                (XCryptographic_EcjPakeContext *)operation->xcryptographic_jpake,
                (char *)operation->buffer, sizeof(operation->buffer));
            if (!result.m_data) return PSA_ERROR_GENERIC_ERROR;
            operation->buffer_length = (size_t)result.m_size;
#else
...
#if defined(MBEDTLS_USE_XCRYPTOGRAPHIC_JPAKE)
            XByteArrayView result = XCryptographic_ecjPakeWriteRoundTwoInto(
                (XCryptographic_EcjPakeContext *)operation->xcryptographic_jpake,
                (char *)operation->buffer, sizeof(operation->buffer));
            if (!result.m_data) return PSA_ERROR_GENERIC_ERROR;
            operation->buffer_length = (size_t)result.m_size;
        ...
        if (step == PSA_JPAKE_X1_STEP_KEY_SHARE ||
            step == PSA_JPAKE_X2S_STEP_KEY_SHARE) {
            operation->buffer_offset = 0;
        }
    ...
    size_t input_length)
{
    ...
    if (PSA_ALG_IS_JPAKE(operation->alg)) {
        ...
        if (step == PSA_JPAKE_X2_STEP_ZK_PROOF) {
#if defined(MBEDTLS_USE_XCRYPTOGRAPHIC_JPAKE)
            if (!XCryptographic_ecjPakeReadRoundOne(
                    (XCryptographic_EcjPakeContext *)operation->xcryptographic_jpake,
                    XByteArrayView_create_data(operation->buffer,
                                               (int64_t)operation->buffer_length))) {
...
#if defined(MBEDTLS_USE_XCRYPTOGRAPHIC_JPAKE)
            if (!XCryptographic_ecjPakeReadRoundTwo(
                    (XCryptographic_EcjPakeContext *)operation->xcryptographic_jpake,
                    XByteArrayView_create_data(operation->buffer,
                                               (int64_t)operation->buffer_length))) {
                return PSA_ERROR_INVALID_SIGNATURE;
    ...
    size_t *output_length)
{
    ...
    if (PSA_ALG_IS_JPAKE(operation->alg)) {
#if defined(MBEDTLS_USE_XCRYPTOGRAPHIC_JPAKE)
        XByteArrayView result = XCryptographic_ecjPakeWriteSharedKeyInto(
            (XCryptographic_EcjPakeContext *)operation->xcryptographic_jpake,
...
psa_status_t mbedtls_psa_pake_abort(mbedtls_psa_pake_operation_t *operation)
{
    ...
    if (PSA_ALG_IS_JPAKE(operation->alg)) {
        ...
        operation->buffer_offset = 0;
#if defined(MBEDTLS_USE_XCRYPTOGRAPHIC_JPAKE)
        XCryptographic_ecjPakeDestroy(
            (XCryptographic_EcjPakeContext *)operation->xcryptographic_jpake);
        operation->xcryptographic_jpake = NULL;
#else
