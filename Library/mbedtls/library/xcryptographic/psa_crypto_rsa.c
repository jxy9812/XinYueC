...
#endif /* MBEDTLS_USE_XCRYPTOGRAPHIC_RSA */

#if defined(MBEDTLS_USE_XCRYPTOGRAPHIC_RSA)
static psa_status_t xcrsa_hash_algorithm(
...
static XCryptographic_RsaKey *xcrsa_import_any(XByteArrayView der)
{
    XCryptographic_RsaKey *key = XCryptographic_rsaImportPublicKey(der);
    if (key == NULL) {
        key = XCryptographic_rsaImportPrivateKey(der);
    }
    return key;
}
#endif /* MBEDTLS_USE_XCRYPTOGRAPHIC_RSA */
    ...
    size_t *key_buffer_length, size_t *bits)
{
    ...
    der = XByteArrayView_create_data(data, (int64_t)data_length);
    if (PSA_KEY_TYPE_IS_KEY_PAIR(attributes->type)) {
        key = XCryptographic_rsaImportPrivateKey(der);
    } else {
        key = XCryptographic_rsaImportPublicKey(der);
    }
    if (key == NULL) {
        return PSA_ERROR_INVALID_ARGUMENT;
    }
    bits_out = PSA_BYTES_TO_BITS(XCryptographic_rsaKeyBytes(key));
    if (bits_out > PSA_VENDOR_RSA_MAX_KEY_BITS) {
        XCryptographic_rsaDestroyKey(key);
        return PSA_ERROR_NOT_SUPPORTED;
    }
    *bits = bits_out;
    ...
    uint8_t *data, size_t data_size, size_t *data_length)
{
    ...
    der = XByteArrayView_create_data(key_buffer, (int64_t)key_buffer_size);
    if (PSA_KEY_TYPE_IS_KEY_PAIR(attributes->type)) {
        key = XCryptographic_rsaImportPrivateKey(der);
    } else {
        key = XCryptographic_rsaImportPublicKey(der);
    ...
    out = XCryptographic_rsaExportPublicKeyInto((char *)data, data_size, key);
    XCryptographic_rsaDestroyKey(key);
    if (out.m_data == NULL) {
        return PSA_ERROR_BUFFER_TOO_SMALL;
    }
    *data_length = (size_t)out.m_size;
    return PSA_SUCCESS;
#else
    ...
    uint8_t *key_buffer, size_t key_buffer_size, size_t *key_buffer_length)
{
    ...
    if (custom_data_length != 0) {
        psa_status_t st = psa_rsa_read_exponent(custom_data, custom_data_length,
                                                &exponent);
        if (st != PSA_SUCCESS) {
            return st;
        }
    ...
    key = XCryptographic_rsaGenerateKey((unsigned) attributes->bits,
                                        (uint32_t) exponent);
    if (key == NULL) {
        return PSA_ERROR_GENERIC_ERROR;
    }
    key_bytes = XCryptographic_rsaKeyBytes(key);
    /* Private-key DER is bounded by roughly 4 * modulus bytes plus header. */
    buf = (uint8_t *) mbedtls_calloc(1, 5 * key_bytes + 64);
    if (buf == NULL) {
        XCryptographic_rsaDestroyKey(key);
        return PSA_ERROR_INSUFFICIENT_MEMORY;
    }
    der = XCryptographic_rsaExportPrivateKeyInto((char *) buf,
                                                 5 * key_bytes + 64, key);
    XCryptographic_rsaDestroyKey(key);
    if (der.m_data == NULL) {
        mbedtls_free(buf);
    ...
    der_len = (size_t) der.m_size;
    if (der_len > key_buffer_size) {
        mbedtls_free(buf);
        return PSA_ERROR_BUFFER_TOO_SMALL;
    }
    memcpy(key_buffer, buf, der_len);
    mbedtls_free(buf);
    *key_buffer_length = der_len;
    return PSA_SUCCESS;
#else
    psa_status_t status;
    mbedtls_rsa_context rsa;
    ...
    uint8_t *signature, size_t signature_size, size_t *signature_length)
{
    ...
    if (PSA_ALG_IS_RSA_PKCS1V15_SIGN(alg)) {
        padding = XCryptographic_RsaPadding_Pkcs1;
    } else if (PSA_ALG_IS_RSA_PSS(alg)) {
        padding = XCryptographic_RsaPadding_Pss;
    } else {
        return PSA_ERROR_INVALID_ARGUMENT;
    }
    status = xcrsa_hash_algorithm(PSA_ALG_SIGN_GET_HASH(alg), &hash_alg);
    if (status != PSA_SUCCESS) {
        return status;
    }
    der = XByteArrayView_create_data(key_buffer, (int64_t) key_buffer_size);
    ...
    if (signature_size < XCryptographic_rsaKeyBytes(key)) {
        XCryptographic_rsaDestroyKey(key);
        return PSA_ERROR_BUFFER_TOO_SMALL;
    }
    hash_view = XByteArrayView_create_data(hash, (int64_t) hash_length);
    sig = XCryptographic_rsaSignHashInto((char *) signature, signature_size,
                                         key, padding, hash_alg, hash_view);
    XCryptographic_rsaDestroyKey(key);
    if (sig.m_data == NULL) {
        return PSA_ERROR_INVALID_ARGUMENT;
    }
    *signature_length = (size_t) sig.m_size;
    return PSA_SUCCESS;
#else
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
    mbedtls_rsa_context *rsa = NULL;
    ...
    const uint8_t *signature, size_t signature_length)
{
    ...
    der = XByteArrayView_create_data(key_buffer, (int64_t) key_buffer_size);
    key = xcrsa_import_any(der);
    if (key == NULL) {
        return PSA_ERROR_INVALID_ARGUMENT;
    }
    hash_view = XByteArrayView_create_data(hash, (int64_t) hash_length);
    sig_view = XByteArrayView_create_data(signature,
                                          (int64_t) signature_length);
    ok = XCryptographic_rsaVerifyHash(key, padding, hash_alg,
                                      hash_view, sig_view);
    XCryptographic_rsaDestroyKey(key);
    return ok ? PSA_SUCCESS : PSA_ERROR_INVALID_SIGNATURE;
#else
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
    mbedtls_rsa_context *rsa = NULL;
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
                                            ...
                                            size_t *output_length)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
#if defined(MBEDTLS_USE_XCRYPTOGRAPHIC_RSA)
    XCryptographic_RsaKey *key = NULL;
    XByteArrayView der;
    ...
    if (PSA_KEY_TYPE_IS_RSA(attributes->type)) {
        ...
        if (alg == PSA_ALG_RSA_PKCS1V15_CRYPT) {
            padding = XCryptographic_RsaPadding_Pkcs1;
            hash_alg = XCryptographicHash_Sha256;
        } else if (PSA_ALG_IS_RSA_OAEP(alg)) {
            padding = XCryptographic_RsaPadding_Oaep;
            status = xcrsa_hash_algorithm(PSA_ALG_RSA_OAEP_GET_HASH(alg),
                                          &hash_alg);
            if (status != PSA_SUCCESS) {
                return status;
            }
        ...
        if (output_size < XCryptographic_rsaKeyBytes(key)) {
            XCryptographic_rsaDestroyKey(key);
            return PSA_ERROR_BUFFER_TOO_SMALL;
        }
        input_view = XByteArrayView_create_data(input, (int64_t) input_length);
        label_view = XByteArrayView_create_data(salt, (int64_t) salt_length);
        out = XCryptographic_rsaEncryptInto((char *) output, output_size, key,
                                            padding, hash_alg, label_view,
                                            input_view);
        XCryptographic_rsaDestroyKey(key);
        if (out.m_data == NULL) {
            return PSA_ERROR_INVALID_ARGUMENT;
        }
        *output_length = (size_t) out.m_size;
        return PSA_SUCCESS;
#else
#if defined(MBEDTLS_PSA_BUILTIN_ALG_RSA_PKCS1V15_CRYPT) || \
        defined(MBEDTLS_PSA_BUILTIN_ALG_RSA_OAEP)
                                            ...
                                            size_t *output_length)
{
    ...
    *output_length = 0;
    ...
    if (attributes->type == PSA_KEY_TYPE_RSA_KEY_PAIR) {
        ...
        if (input_length != XCryptographic_rsaKeyBytes(key)) {
            XCryptographic_rsaDestroyKey(key);
            return PSA_ERROR_INVALID_ARGUMENT;
        }
        if (output_size < XCryptographic_rsaKeyBytes(key)) {
        ...
        input_view = XByteArrayView_create_data(input, (int64_t) input_length);
        label_view = XByteArrayView_create_data(salt, (int64_t) salt_length);
        out = XCryptographic_rsaDecryptInto((char *) output, output_size, key,
                                            padding, hash_alg, label_view,
                                            input_view, &plain_len);
        XCryptographic_rsaDestroyKey(key);
        if (out.m_data == NULL) {
            return PSA_ERROR_INVALID_ARGUMENT;
        }
