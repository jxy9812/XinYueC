...
#if defined(MBEDTLS_USE_XCRYPTOGRAPHIC_FFDH)
#include "XCryptographic.h"
#endif /* MBEDTLS_USE_XCRYPTOGRAPHIC_FFDH */
    ...
    size_t *data_length)
{
...
#if defined(MBEDTLS_USE_XCRYPTOGRAPHIC_FFDH)
    {
        ...
        view = XCryptographic_ffdhExportPublicKeyInto(
            out, key_len,
            (const char *)key_buffer, key_buffer_size);
        if (view.m_data == NULL) {
            return PSA_ERROR_INVALID_ARGUMENT;
        }
        *data_length = key_len;
...
#else /* MBEDTLS_USE_XCRYPTOGRAPHIC_FFDH */
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    uint8_t *key_buffer, size_t key_buffer_size, size_t *key_buffer_length)
{
    (void) attributes;
#if defined(MBEDTLS_USE_XCRYPTOGRAPHIC_FFDH)
    if (!XCryptographic_ffdhGeneratePrivateKeyInto(
    ...
    size_t *shared_secret_length)
{
...
#if defined(MBEDTLS_USE_XCRYPTOGRAPHIC_FFDH)
    {
        XByteArrayView view = XCryptographic_ffdhAgreeInto(
            (char *)shared_secret, calculated_shared_secret_size,
            (const char *)key_buffer, key_buffer_size,
            (const char *)peer_key, peer_key_length);
        if (view.m_data == NULL) {
            return PSA_ERROR_INVALID_ARGUMENT;
        }
     ...
     * 3525 (since v1.4, not yet supported in psa-crypto as of writing this
