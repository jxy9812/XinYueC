...
#if defined(MBEDTLS_USE_XCRYPTOGRAPHIC_KDF) || \
    defined(MBEDTLS_USE_XCRYPTOGRAPHIC_JPAKE)
                                                    ...
                                                    size_t bits)
{
    ...
    switch (type) {
        ...
        case PSA_KEY_TYPE_AES:
            /* XTS uses a concatenation of two AES keys: 256 bits for
             * AES-128-XTS and 512 bits for AES-256-XTS. Other AES algorithms
             * remain constrained by their respective driver setup paths. */
...
#if defined(MBEDTLS_USE_XCRYPTOGRAPHIC_KDF)
static int xcryptographic_kdf_view(const uint8_t *data, size_t size,
...
static psa_status_t xcryptographic_kdf_hash_algorithm(
    psa_algorithm_t alg, XCryptographicHash_Algorithm *result);

static psa_status_t xcryptographic_kdf_hkdf_expand_read(
    psa_hkdf_key_derivation_t *hkdf, psa_algorithm_t hash_alg,
    uint8_t *output, size_t output_length)
{
    size_t consumed;
    size_t total;
    uint8_t *tmp;
    XByteArrayView prk_view;
    XByteArrayView info_view;
    XByteArrayView result;

    if (hkdf->state == HKDF_STATE_KEYED) {
        consumed = 0;
    } else {
        consumed = ((size_t) hkdf->block_number << 8) | hkdf->offset_in_block;
    }
    size_t hash_length = PSA_HASH_LENGTH(hash_alg);
    XCryptographicHash_Algorithm xhash;
    if (xcryptographic_kdf_hash_algorithm(hash_alg, &xhash) != PSA_SUCCESS ||
        consumed > 255u * hash_length || output_length > 255u * hash_length - consumed) {
        return PSA_ERROR_INVALID_ARGUMENT;
    }
    total = consumed + output_length;
    tmp = mbedtls_calloc(1, total);
    if (tmp == NULL) {
        return PSA_ERROR_INSUFFICIENT_MEMORY;
    }
    if (!xcryptographic_kdf_view(hkdf->prk, hash_length, &prk_view) ||
        !xcryptographic_kdf_view(hkdf->info, hkdf->info_length, &info_view)) {
        mbedtls_free(tmp);
        return PSA_ERROR_INVALID_ARGUMENT;
    }
    result = XCryptographic_hkdfExpandInto(
        prk_view, info_view, xhash, total, (char *) tmp, total);
    if (!result.m_data) {
        mbedtls_free(tmp);
        return PSA_ERROR_GENERIC_ERROR;
    }
    memcpy(output, tmp + consumed, output_length);
    mbedtls_free(tmp);
    consumed = total;
    hkdf->block_number = (uint8_t) (consumed >> 8);
    hkdf->offset_in_block = (uint8_t) (consumed & 0xff);
    hkdf->state = HKDF_STATE_OUTPUT;
    return PSA_SUCCESS;
}

static psa_status_t xcryptographic_kdf_tls12_prf_read(
    ...
    psa_algorithm_t alg, uint8_t *output, size_t output_length)
{
    ...
    size_t total;
    uint8_t *tmp;
    XByteArrayView secret_view;
    XByteArrayView label_view;
    XByteArrayView seed_view;
    XByteArrayView result;
    XCryptographicHash_Algorithm xhash;
    size_t hash_length = PSA_HASH_LENGTH(PSA_ALG_TLS12_PRF_GET_HASH(alg));

    if (tls12_prf->state == PSA_TLS12_PRF_STATE_LABEL_SET) {
        consumed = 0;
    } else {
        consumed = ((size_t) tls12_prf->block_number << 8) |
                   tls12_prf->left_in_block;
    }
    if (xcryptographic_kdf_hash_algorithm(PSA_ALG_TLS12_PRF_GET_HASH(alg), &xhash) != PSA_SUCCESS ||
        consumed > 255u * hash_length || output_length > 255u * hash_length - consumed) {
        return PSA_ERROR_INVALID_ARGUMENT;
    }
    total = consumed + output_length;
    ...
    if (!xcryptographic_kdf_view(tls12_prf->secret, tls12_prf->secret_length,
                                 &secret_view) ||
        !xcryptographic_kdf_view(tls12_prf->label, tls12_prf->label_length,
                                 &label_view) ||
        !xcryptographic_kdf_view(tls12_prf->seed, tls12_prf->seed_length,
                                 &seed_view)) {
        mbedtls_free(tmp);
        return PSA_ERROR_INVALID_ARGUMENT;
    }
    /* For TLS 1.2 PSK-to-MS, the PSA input handler already builds the
     * premaster secret and stores it in tls12_prf->secret, so the same
     * XCryptographic TLS 1.2 PRF path applies to both PRF and PSK-to-MS. */
    (void) alg;
    result = XCryptographic_tls12PrfInto(
        secret_view, label_view, seed_view, xhash, total, (char *) tmp, total);
    if (!result.m_data) {
        mbedtls_free(tmp);
        return PSA_ERROR_GENERIC_ERROR;
    }
    ...
    mbedtls_free(tmp);
    consumed = total;
    tls12_prf->block_number = (uint8_t) (consumed >> 8);
    tls12_prf->left_in_block = (uint8_t) (consumed & 0xff);
    tls12_prf->state = PSA_TLS12_PRF_STATE_OUTPUT;
    return PSA_SUCCESS;
}

static psa_status_t xcryptographic_kdf_pbkdf2_aes_cmac_read(
    ...
    uint8_t *output, size_t output_length)
{
    ...
    size_t total;
    uint8_t *tmp;
    XByteArrayView password_view;
    XByteArrayView salt_view;
    XByteArrayView result;

    if (pbkdf2->state == PSA_PBKDF2_STATE_PASSWORD_SET) {
        consumed = 0;
    } else {
        consumed = ((size_t) pbkdf2->block_number << 8) |
                   pbkdf2->bytes_used;
    }
    if (pbkdf2->input_cost > UINT32_MAX ||
        output_length > SIZE_MAX - consumed) {
        return PSA_ERROR_INVALID_ARGUMENT;
    }
    total = consumed + output_length;
    tmp = mbedtls_calloc(1, total);
    ...
    if (!xcryptographic_kdf_view(pbkdf2->password, pbkdf2->password_length,
                                 &password_view) ||
        !xcryptographic_kdf_view(pbkdf2->salt, pbkdf2->salt_length,
                                 &salt_view)) {
        mbedtls_free(tmp);
        return PSA_ERROR_INVALID_ARGUMENT;
    }
    result = XCryptographic_pbkdf2AesCmacPrf128Into(
        password_view, salt_view, (uint32_t) pbkdf2->input_cost,
        total, (char *) tmp, total);
    if (!result.m_data) {
        mbedtls_free(tmp);
        return PSA_ERROR_GENERIC_ERROR;
    }
    ...
    mbedtls_free(tmp);
    consumed = total;
    pbkdf2->block_number = (uint32_t) (consumed >> 8);
    pbkdf2->bytes_used = (uint8_t) (consumed & 0xff);
...
static psa_status_t xcryptographic_kdf_pbkdf2_hmac_read(
    psa_pbkdf2_key_derivation_t *pbkdf2,
    ...
    uint8_t *output, size_t output_length)
{
    ...
    status = xcryptographic_kdf_hash_algorithm(
        PSA_ALG_PBKDF2_HMAC_GET_HASH(kdf_alg), &hash_algorithm);
    if (status != PSA_SUCCESS || pbkdf2->input_cost > UINT32_MAX) {
        return PSA_ERROR_NOT_SUPPORTED;
    }
    if (pbkdf2->state == PSA_PBKDF2_STATE_PASSWORD_SET) {
        consumed = 0;
    } else if (pbkdf2->state == PSA_PBKDF2_STATE_OUTPUT) {
        consumed = ((size_t) pbkdf2->block_number << 8) |
                   pbkdf2->bytes_used;
    } else {
        return PSA_ERROR_BAD_STATE;
    }
    if (output_length > SIZE_MAX - consumed) {
        return PSA_ERROR_INVALID_ARGUMENT;
    }
    total = consumed + output_length;
    tmp = mbedtls_calloc(1, total);
    ...
    result = XCryptographic_pbkdf2HmacInto(
        password_view, salt_view, (uint32_t) pbkdf2->input_cost,
        hash_algorithm, total, (char *) tmp, total);
    if (!result.m_data) {
        mbedtls_free(tmp);
        return PSA_ERROR_GENERIC_ERROR;
    }
    memcpy(output, tmp + consumed, output_length);
    mbedtls_free(tmp);
    pbkdf2->block_number = (uint32_t) (total >> 8);
    pbkdf2->bytes_used = (uint8_t) (total & 0xff);
    pbkdf2->state = PSA_PBKDF2_STATE_OUTPUT;
    return PSA_SUCCESS;
}
#endif /* MBEDTLS_USE_XCRYPTOGRAPHIC_KDF */
                                                 ...
                                                 size_t output_length)
{
    ...
    if (XCRYPTOGRAPHIC_HKDF_HMAC_ON &&
        !PSA_ALG_IS_HKDF_EXTRACT(kdf_alg)) {
        return xcryptographic_kdf_hkdf_expand_read(
            hkdf, hash_alg, output, output_length);
    }
#endif
    hkdf->state = HKDF_STATE_OUTPUT;
    ...
    size_t output_length)
{
    ...
    if (XCRYPTOGRAPHIC_TLS12_PRF_HMAC_ON &&
        (tls12_prf->state == PSA_TLS12_PRF_STATE_LABEL_SET ||
         tls12_prf->state == PSA_TLS12_PRF_STATE_OUTPUT)) {
        return xcryptographic_kdf_tls12_prf_read(
            tls12_prf, alg, output, output_length);
    }
#endif
...
#if defined(MBEDTLS_USE_XCRYPTOGRAPHIC_JPAKE)
    {
        XByteArrayView result = XCryptographicHash_hashInto(
            (char *) output, output_length, (const char *) ecjpake->data,
            PSA_TLS12_ECJPAKE_TO_PMS_DATA_SIZE, XCryptographicHash_Sha256);
        if (result.m_data != output || result.m_size != (int64_t) output_length) {
            return PSA_ERROR_GENERIC_ERROR;
        }
    }
    return PSA_SUCCESS;
...
#if defined(MBEDTLS_USE_XCRYPTOGRAPHIC_KDF)
    if (PSA_ALG_IS_PBKDF2_HMAC(kdf_alg) && XCRYPTOGRAPHIC_PBKDF2_HMAC_ON &&
        (pbkdf2->state == PSA_PBKDF2_STATE_PASSWORD_SET ||
         pbkdf2->state == PSA_PBKDF2_STATE_OUTPUT)) {
        XCryptographicHash_Algorithm xcryptographic_hash;
        if (xcryptographic_kdf_hash_algorithm(
                PSA_ALG_PBKDF2_HMAC_GET_HASH(kdf_alg),
                &xcryptographic_hash) == PSA_SUCCESS) {
            return xcryptographic_kdf_pbkdf2_hmac_read(
                pbkdf2, kdf_alg, output, output_length);
        }
    }
    if (kdf_alg == PSA_ALG_PBKDF2_AES_CMAC_PRF_128 &&
        XCRYPTOGRAPHIC_PBKDF2_AES_CMAC_PRF_128_ON &&
        (pbkdf2->state == PSA_PBKDF2_STATE_PASSWORD_SET ||
         pbkdf2->state == PSA_PBKDF2_STATE_OUTPUT)) {
        return xcryptographic_kdf_pbkdf2_aes_cmac_read(
            pbkdf2, output, output_length);
    }
#endif
...
typedef struct {
    ...
    const uint8_t *order;
    size_t order_len;
} psa_weierstrass_curve_info_t;

static const uint8_t order_secp256r1[] = {
    0xff,0xff,0xff,0xff,0x00,0x00,0x00,0x00,0xff,0xff,0xff,0xff,
    0xff,0xff,0xff,0xff,0xbc,0xe6,0xfa,0xad,0xa7,0x17,0x9e,0x84,
    0xf3,0xb9,0xca,0xc2,0xfc,0x63,0x25,0x51,
};
static const uint8_t order_secp384r1[] = {
    0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,
    0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,
    0xc7,0x63,0x4d,0x81,0xf4,0x37,0x2d,0xdf,0x58,0x1a,0x0d,0xb2,
    0x48,0xb0,0xa7,0x7a,0xec,0xec,0x19,0x6a,0xcc,0xc5,0x29,0x73,
};
static const uint8_t order_secp521r1[] = {
...
static const uint8_t order_brainpoolP256r1[] = {
...
static const uint8_t order_brainpoolP384r1[] = {
...
static const uint8_t order_brainpoolP512r1[] = {
...
static const uint8_t order_secp256k1[] = {
    0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,
    0xff,0xff,0xff,0xfe,0xba,0xae,0xdc,0xe6,0xaf,0x48,0xa0,0x3b,
    0xbf,0xd2,0x5e,0x8c,0xd0,0x36,0x41,0x41,
};

static const psa_weierstrass_curve_info_t weierstrass_curves[] = {
    { PSA_ECC_FAMILY_SECP_R1, 256, 256, order_secp256r1, sizeof(order_secp256r1) },
    { PSA_ECC_FAMILY_SECP_R1, 384, 384, order_secp384r1, sizeof(order_secp384r1) },
    { PSA_ECC_FAMILY_SECP_R1, 521, 521, order_secp521r1, sizeof(order_secp521r1) },
    { PSA_ECC_FAMILY_BRAINPOOL_P_R1, 256, 256, order_brainpoolP256r1, sizeof(order_brainpoolP256r1) },
    { PSA_ECC_FAMILY_BRAINPOOL_P_R1, 384, 384, order_brainpoolP384r1, sizeof(order_brainpoolP384r1) },
    { PSA_ECC_FAMILY_BRAINPOOL_P_R1, 512, 512, order_brainpoolP512r1, sizeof(order_brainpoolP512r1) },
    { PSA_ECC_FAMILY_SECP_K1, 256, 256, order_secp256k1, sizeof(order_secp256k1) },
};
    ...
    )
{
    ...
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
    const psa_weierstrass_curve_info_t *info = NULL;
    size_t m = 0;
    size_t m_bytes = 0;
    size_t i;
    ...
    for (i = 0; i < sizeof(weierstrass_curves) / sizeof(weierstrass_curves[0]); i++) {
        if (weierstrass_curves[i].family == curve &&
            weierstrass_curves[i].bits == bits) {
            info = &weierstrass_curves[i];
            break;
        }
    }
    ...
    /* N is the boundary of the private key domain (info->order). */
    ...
    MBEDTLS_MPI_CHK(mbedtls_mpi_read_binary(&diff_N_2, info->order, info->order_len));
    MBEDTLS_MPI_CHK(mbedtls_mpi_sub_int(&diff_N_2, &diff_N_2, 2));
...
#if defined(MBEDTLS_USE_XCRYPTOGRAPHIC_KDF)
        XCryptographicHash_Algorithm xcryptographic_hash;
        if (xcryptographic_kdf_hash_algorithm(hash_alg, &xcryptographic_hash) == PSA_SUCCESS) {
            XByteArrayView result = XCryptographicHash_hashInto(
                (char *) output, PSA_HMAC_MAX_HASH_BLOCK_SIZE,
                (const char *) input, input_len, xcryptographic_hash);
            if (!result.m_data) {
                return PSA_ERROR_GENERIC_ERROR;
            }
            *output_len = (size_t) result.m_size;
            return PSA_SUCCESS;
        }
#endif
