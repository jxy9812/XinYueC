...
#if defined(MBEDTLS_USE_XCRYPTOGRAPHIC_LEGACY_MD)
static int md_xcryptographic_algorithm(mbedtls_md_type_t type,
                                        XCryptographicHash_Algorithm *algorithm)
{
    if (!algorithm) return 0;
    switch (type) {
        case MBEDTLS_MD_MD5: *algorithm = XCryptographicHash_Md5; return 1;
        case MBEDTLS_MD_RIPEMD160: *algorithm = XCryptographicHash_Ripemd160; return 1;
        case MBEDTLS_MD_SHA1: *algorithm = XCryptographicHash_Sha1; return 1;
        case MBEDTLS_MD_SHA224: *algorithm = XCryptographicHash_Sha224; return 1;
        case MBEDTLS_MD_SHA256: *algorithm = XCryptographicHash_Sha256; return 1;
        case MBEDTLS_MD_SHA384: *algorithm = XCryptographicHash_Sha384; return 1;
        case MBEDTLS_MD_SHA512: *algorithm = XCryptographicHash_Sha512; return 1;
        case MBEDTLS_MD_SHA3_224: *algorithm = XCryptographicHash_Sha3_224; return 1;
        case MBEDTLS_MD_SHA3_256: *algorithm = XCryptographicHash_Sha3_256; return 1;
        case MBEDTLS_MD_SHA3_384: *algorithm = XCryptographicHash_Sha3_384; return 1;
        case MBEDTLS_MD_SHA3_512: *algorithm = XCryptographicHash_Sha3_512; return 1;
        default: return 0;
    }
}

static XCryptographicHash *md_xcryptographic_context(void *ctx)
{
    return (XCryptographicHash *)ctx;
}

static int md_xcryptographic_finish(XCryptographicHash *hash,
                                     unsigned char *output, size_t output_size)
{
    XByteArrayView result;
    if (!hash || !output) return MBEDTLS_ERR_MD_BAD_INPUT_DATA;
    result = XCryptographicHash_resultView(hash);
    if (!result.m_data || result.m_size < 0 || (size_t)result.m_size < output_size) {
        return MBEDTLS_ERR_MD_BAD_INPUT_DATA;
    }
    memcpy(output, result.m_data, output_size);
    return 0;
}
#endif
...
int mbedtls_md_setup(mbedtls_md_context_t *ctx, const mbedtls_md_info_t *md_info, int hmac)
{
    ...
    {
        XCryptographicHash_Algorithm algorithm;
        if (!md_xcryptographic_algorithm(md_info->type, &algorithm)) {
            return MBEDTLS_ERR_MD_BAD_INPUT_DATA;
        }
...
#if defined(MBEDTLS_USE_XCRYPTOGRAPHIC_LEGACY_MD)
    if (!ctx || !ctx->md_ctx || (ilen != 0 && !input) || ilen > (size_t)INT64_MAX) {
        return MBEDTLS_ERR_MD_BAD_INPUT_DATA;
    }
    if (ilen != 0) {
        XCryptographicHash_addData_2(
            md_xcryptographic_context(ctx->md_ctx),
               ...
               unsigned char *output)
{
...
#if defined(MBEDTLS_USE_XCRYPTOGRAPHIC_LEGACY_MD)
    {
        XCryptographicHash_Algorithm algorithm;
        XByteArrayView result;
        if (!output || (ilen != 0 && !input) || ilen > (size_t)INT64_MAX ||
            !md_xcryptographic_algorithm(md_info->type, &algorithm)) {
            return MBEDTLS_ERR_MD_BAD_INPUT_DATA;
        }
        result = XCryptographicHash_hashInto(
            (char *)output, md_info->size,
