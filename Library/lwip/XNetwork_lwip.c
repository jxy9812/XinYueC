...
typedef struct XNetworkSocketPrivateLwip {
    ...
    size_t writeFinished;          /* 已完成写入的累积字节数（待消费） */
    size_t lastWriteFinished;      /* 上次事件消费的写入字节数快照 */
...
static void tcpErrCb(void* arg, err_t err) {
    ...
    if (s) {
        ...
        s->tpcb = NULL;
        s->connected = false;  /* 错误后标记为未连接，确保 Connect 处理路径走失败分支 */
    }
}
...
bool XNetwork_socketHandleEvent(XNetworkSocketPrivate* priv, void* event) {
    ...
    if (s->hasReadData) {
        s->hasReadData = false;
        s->rxTotal = s->rxPos;  /* 快照当前缓冲区中可读数据总量 */
        hasEvent = true;
    }
    if (s->hasWriteDone) {
        s->hasWriteDone = false;
        s->lastWriteFinished = s->writeFinished;  /* 快照已完成写入量 */
        s->writeFinished = 0;                      /* 重置累积器 */
        hasEvent = true;
    }
