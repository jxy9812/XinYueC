...
void Vector_free(const struct vector* this_vector)//释放内存
{
	VECTOR* vector = (VECTOR*)this_vector;
	Vector_clear(vector);
	free(vector);
...
void Vector_swap(struct vector* vec1, struct vector* vec2)//交换两个同类型向量的数据
{
	...
	ContainerObject_swap(&vector1->object, &vector2->object);
}
