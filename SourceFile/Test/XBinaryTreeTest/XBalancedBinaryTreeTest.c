...
void TreeNodeBalanceTest()
{
	...
	XBBTreeNode* root = XBBTree_insert(NULL, XLess_int,LPa++, sizeof(int));
	...
	int findVal = 456;
	XBBTreeNode* findRet = XBBTree_findData(root, XLess_int, XEquality_int,&findVal);
	if(findRet!=NULL)
	...
	{
		XBBTree_erase(&root, XLess_int, XEquality_int, a+i, sizeof(int));
	}
