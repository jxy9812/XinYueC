...
//插入函数
Node* List_push_front(list* li, void* x);// 链表头部增加一个元素X
Node* List_push_back(list* li, void* x);// 链表尾部增加一个元素X
void  List_insert_front_p(list* li, Node* pval, ...);//(list*li, const void* p, const void* x, const int n) 链表中指向元素p前增加x元素n个,不填n默认1个(单次调用最多插入1000个，溢出均为1个)
void List_insert_front_int(list* li, int n, ...);//(list*li, int n, const void* x, const int n) 链表中下标n的节点前增加x元素n个,不填n默认1个(单次调用最多插入1000个，溢出均为1个)
void  List_insert(list* li, Node* pval, const void* p1, const void* p2);// 链表中指向元素p前插入另一个相同类型数组[p1,p2]间的数据，数组传递用指针
//删除函数
void  List_pop_front(list* li);//删除链表中第一个元素
void  List_pop_back(list* li);//删除链表中最后一个元素
void  List_erase_p(list* li, const Node* p1, const Node* p2);//删除指定元素区间内的数据(包括其本身)，传入其节点指针地址，搭配find函数查找返回指针最佳，删除一个时请输入相同的指针
...
//遍历函数
Node* List_at(const list* li, int i);// 想象成数组，输入下标返回元素节点的指针
Node* List_front(list* li);//返回链表头指针，指向第一个节点指针
Node* List_back(list* li);//返回链表尾指针，指向链表最后一个节点指针
Node* List_find(const list* li, const void* val);//查找数据，返回找到的指针，没有返回NULL
//判断函数
