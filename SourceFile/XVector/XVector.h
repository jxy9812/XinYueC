...
// 向量尾部增加一个元素X
void XVector_Push_Back(XVector* this_vector, void*LValue);
// 向量中指向元素p前增加一个元素x
void XVector_insert_front(XVector* this_vector, const void* p, const void*LValue);
// 向量中指向元素p前增加n个相同的元素x
void XVector_insert_nfront(XVector* this_vector, const void* p, const int n, const void*LValue);
...
//排序
void  XVector_sort(XVector* this_vector, int (*Sort)(const void* LPrevValue, const void* LNextValue));
//释放内存
