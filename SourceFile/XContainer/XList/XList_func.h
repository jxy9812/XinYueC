...
//插入函数
// 链表头部增加一个元素X
struct Node* XList_push_front(struct XList* this_list, void* LValue);
// 链表尾部增加一个元素X
struct Node* XList_push_back(struct XList* this_list, void* LValue);
//(list*li, const void* p, const void*LValue, const int n) 链表中指向元素p前增加x元素n个,不填n默认1个(单次调用最多插入1000个，溢出均为1个)
void  XList_insert_front_p(struct XList* this_list, struct Node* pval, ...);
...
void  XList_insert(struct XList* this_list, struct Node* pval, const void* p1, const void* p2);
//删除函数
//删除链表中第一个元素
...
void  XList_erase_p(struct XList* this_list, const struct Node* p1, const struct Node* p2);
...
//遍历函数
// 想象成数组，输入下标返回元素节点的指针
struct Node* XList_at(const struct XList* this_list, int i);
//返回链表头指针，指向第一个节点指针
struct Node* XList_front(struct XList* this_list);
//返回链表尾指针，指向链表最后一个节点指针
struct Node* XList_back(struct XList* this_list);
//查找数据，返回找到的指针，没有返回NULL
struct Node* XList_find(const struct XList* this_list, bool (*find)(const struct Node* node, const void* val), const void* findVal);
//判断函数
//检测list内是否为空，空为真 O(1)
