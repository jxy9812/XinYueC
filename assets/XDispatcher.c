...
#if VTABLE_ISSTACK
        XVTABLE_STACK_INIT_DEFAULT(XDispatcher)
#else
...
void XDispatcher_init(XDispatcher* dispatcher, XSchedulePolicy policy)
{
    ...
    dispatcher->last_sched_time = XDateTime_currentMSecsSinceEpoch(); // 初始化为当前毫秒时间
...
void XScheduleEntity_init(XScheduleEntity* entity, void* data, int priority, XSchedulePolicy policy) {
    ...
    entity->vruntime = 0;
    entity->runtime = 0;
    // RR策略默认时间片为100ms（与XDateTime_currentMSecsSinceEpoch单位一致）
...
static void VXDispatcher_schedule(XDispatcher* dispatcher) {
    ...
    XMutex_lock(dispatcher->mutex);
    uint64_t current_time = XDateTime_currentMSecsSinceEpoch(); // 当前时间（毫秒）
